...
function action(mode, type, selection) {
    if (mode == -1) {
        ...
        if (status == 0) {
            ...
            add += "" + xxx + "-当前下注押金:#b<#e#r 点卷赌博 #n#b>#b<#e#r " + cost + " 点卷#n#b >#k\r\n ";
            add += "" + xxx + "-当前您拥有点卷：" + cm.getPlayer().getCSPoints(1) + ".#k\r\n";
            add += "#L0#" + ttt + "-[#r加注#k]#l\r\n\r\n";
                ...
                if (jilv == 0) {} else if (jilv != 0) {
                    if (cm.getPlayer().getCSPoints(1) < cost) {
                        ...
                        if (jiaru == 0) {
                            costa = cost * jilv
                            cm.gainNX(costa);
                            //cm.getC().getChannelServer().broadcastPacket(Packages.tools.MaplePacketCreator.serverNotice(0x1B, cm.getC().getChannel(), "[博彩公告]" + " : 恭喜玩家：" + cm.getPlayer().getName() + " 在金币博彩中赢得" + costa + "点卷。"));
