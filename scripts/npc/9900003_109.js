...
function action(mode, type, selection) {
    if (mode == -1) {
        ...
        if (mode == 1)
            ...
            status--;
        if (status == 0) {
            cm.sendSimple("#e<森林BOSS挑战>#n\r\n为了强大的埃苏莱布斯武器，想要带领队友进入森林挑战群体BOSS首领?，你们准备好了吗？如果准备好了，请让你们的队长和我谈话……。\r\n#b#L0#我想执行组队任务。#l\r\n#L3#我要兑换埃苏莱布斯武器。#l\r\n#L1#我想听一下说明。#l\r\n#L4##r我要花费点卷重置次数。")
        } else if (status == 1) {
            if (selection == 0) {
				if (cm.getBossLog("森林保卫") >= maxPlay) {
                ...
                if (cm.getParty() == null) { // 没有组队
					...
					if (getBossLog(idx)>=maxPlay) {
						cm.sendOk("队伍中有玩家已经参与过该副本10次，无法再进入，请踢出该玩家。");
						cm.dispose();
						return;
					}
                ...
                cm.sendOk("副本总共有七个关卡，每天可进行10次副本，如果火力不足够，是会自动脱离战场的。击败最后关卡的就能开启#e#b神秘宝箱#n#k，里面有传说中的#b埃苏莱布斯武器制作材料#k，还有各种新奇的椅子和时装；开启宝箱后，玩家通过掷点的方式来赢取奖励。掷点时，你需要在10秒钟内做出需求选择，否则将会被强制下线。\r\n进入副本前，请确认你的组队员：\r\n\r\n#b1、组队员必须要" + minPartySize + "人以上，" + maxPartySize + "人以下。\r\n2、组队员等级必须要在" + minLevel + "级以上。\r\n\r\n（#r如果仍然错误, 重新下线,再登陆 或者请重新组队。#k#b）");
                cm.dispose();
            } else if (selection == 2) {
                cm.warp(910000000, 0)
                cm.sendOk("我等着更加强大的你……")
				...
				if (cm.haveItem(4310150,50)) {
					if (cm.getSpace(2) > 1) {
						...
						cm.sendOk("成功兑换了1个#r#z2432255##k");
					} else {
						cm.sendOk("兑换失败，消耗栏包裹不足，请检查！");
					}
					cm.dispose();
				} else {
					cm.sendOk("您没有集齐50个#b#z4310150##k，无法兑换#b#z2432255##k，打开后可获得一把#r极品埃苏莱布斯武器#k。");
					cm.dispose();
				}
			} else if (selection == 4) {
				cm.sendYesNo("你是否需要花费#r10000#k点卷重置该副本？");
			}
        } else if (status == 2) {
...
function getBossLog(idx) {
	...
	var sql = "SELECT max(count) as maxcount FROM bosslog where bossid = '森林保卫' and characterid in (" + idStr + ") and to_days(time) = to_days(now());";
