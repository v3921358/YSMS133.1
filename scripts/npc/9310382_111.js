...
function action(mode, type, selection) {
	if (mode == -1) {
		...
		if (mode == 1)
			...
			status--;
		if (status == 0) {
			cm.sendSimple("亲爱的#r#h ##k您好，我是会员伤害上限突破系统:\r\n#r#L1#伤害上限突破简介#l\r\n\r\n#L3#[武器]伤害上限突破-模式(1) (#k目前状态：#r火爆内容#b)#l");
		} else if (status == 1) {
			if (selection == 1) {
				...
				if (cm.getBossLog("点卷突破1", 1) <= 1000) {
					if (cm.haveItem(4000463, 100)) {
						...
						if (cm.changeLimitBreak(1000000)) {
							...
							cm.sendOk("#b伤害上限突破成功.\r\n\r\n本次追加伤害为：#r1000000#b.");
							cm.worldSpouseMessage(0x20, "[伤害突破] 玩家 " + cm.getChar().getName() + " 在我要变强中使用 国庆币 让武器伤害上限突破成功 本次追加 100W 伤害值。");
						} else {
							cm.sendOk("#b突破失败.\r\n系统为检测到角色身上装备武器.");
						}
						cm.dispose();
