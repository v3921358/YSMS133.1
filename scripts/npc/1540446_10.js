...
function action(mode, type, selection) {
	...
	if (cm.getPlayer().getClient().getChannel() == 2 || cm.getPlayer().getClient().getChannel() == 3) {
		...
		if (cm.checkPartyEventCount("斯乌",2)){
				...
				cm.setPartyEventCount("斯乌");
                                em.startInstance(cm.getParty(), cm.getMap());
								cm.worldSpouseMessage(0x25, "[系统消息] : 玩家 "+cm.getPlayer().getName()+"  带领他的队伍进入了黑色天堂内部。");
                                cm.dispose();
                                return;
                            } else {
                                cm.sendOk("副本中已经有人了，请稍等！");
                            }
                        }
                        cm.dispose();
                    } else {
			...
			cm.dispose();
        		...
        		cm.dispose();
        		cm.sendOk("只有在2频道或3频道才可以参加[黑色天堂]挑战终极斯乌。");
	}
    }
}