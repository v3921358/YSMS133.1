...
function action(mode, type, selection) {
	if (mode == -1) {
		...
		var selStr = "#e#b您当前在本世界在线时间： #r" + cm.getPlayer().getTodayOnlineTime() + " #b分钟。\r\n";
			...
			selStr += "#b当前点卷: #r" + cm.getNX(1) + " #b点\t\t当前抵用卷: #r" + cm.getNX(2) + " #b点#n\r\n\r\n #e#b请选择购买天数：#n#k\r\n";
            //selStr +="#L1##r"+aaa+" 随身服务包10元宝/一天权 [详情点击查看]#l#k\r\n"
			selStr +="#L2##r"+aaa+" 随身服务包150元宝/10天权 [详情点击查看]#l#k\r\n"
			//selStr +="#L3##r"+aaa+" 超级实惠理财服务50W点卷/30天权 [详情点击查看]#l#k\r\n";
			selStr +="#L4##r"+aaa+" 随身服务包260元宝/30天权 [详情点击查看]#l#k\r\n";
			...
			if (selection == 1) {
				...
				typed=4;
				cm.sendYesNo(head + "#e#d超级实惠理财服务一月权：#n#k\r\n- #e#r提示:#k#n  #r#z2430865# 260元宝/30天#k\r\n- #e#d随身服务特权：#n#k\r\n\t\t#b拥有全服上线提示、独特聊天颜色。\r\n- #e#d详细说明：#n#k\r\n\t\t办理后会扣掉您260元宝，并且每天可以领取 [10000] 点卷, 在30天内享有三倍经验以及双倍爆率，每天领取20个[高级神奇魔方、大师级神奇魔方、防暴卷轴、祝福卷轴]。免费更换发型脸型染色以及快速洗血。领取每日金币. 极品装备卷轴商店,免费租借稀有椅子，理财钱庄领取元宝等\r\n#r亲们理财等于的白送给大家的，心动不如行动#k\r\n- #e#d管理提示：#n#b点是进行购买。点否返回上一页.#k");	
			} else if (selection == 5) {
				typed=5;
			...
			cm.dispose();
                ...
                if (cm.haveItem(2430865) < 1 && cm.getSpace(2) > 2 && cm.getHyPay(1) >= 150 && cm.getSevenDayPayLog(1).get(0) >= 150) {
			...
			cm.dispose();
                ...
                if (cm.haveItem(2430865) < 1 && cm.getSpace(2) > 2 && cm.getPlayer().getCSPoints(1) >= 1000000) {
			...
			cm.dispose();
                ...
                if (cm.haveItem(2430865) < 1 && cm.getSpace(2) > 2 && cm.getHyPay(1) >= 260 && cm.getSevenDayPayLog(1).get(0) >= 260) {
			cm.gainItem(2430865,1,30);
			cm.addHyPay(260);
