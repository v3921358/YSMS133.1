...
var onlineTime = Array(30,60,120,360,480,600,720,888,1000,1200); //填写时间，单位:分钟,即180分钟和480分钟可以领取一次
var mPoints = Array(1000, 2000, 3000, 4000, 5000, 6000, 7000, 8000, 9000, 10000);  //填写抵用卷，用逗号分隔，对应上面的分钟数
...
function action(mode, type, selection) {
	...
	if (status == 0) {
		var text = "你好，欢迎来到抵用券领取中心，如果你满足条件，就可以领取以下抵用券奖励。#r（此活动维持5天）#k\r\n";
		for(var key in onlineTime) {
			text +="#L"+key+"#"+icon+" #b在线 #r"+onlineTime[key]+" #b分钟领取 #r"+mPoints[key]+" #b抵用券#l\r\n";
		}
		cm.sendSimple(text);
	} else if (status == 1){
		var sel = selection;
		...
		if (cm.getBossLog("在线抵用"+sel) == 0) {
			if (cm.getPlayer().getTodayOnlineTime() >= needTime) {
				...
				cm.worldSpouseMessage(0x23, "『在线抵用』 : 玩家 "+cm.getPlayer().getName()+" 领取了 "+needTime+"分钟 "+points+" 抵用卷奖励。");
			} else {
