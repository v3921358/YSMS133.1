...
var rewardItem = new Array(
						new Array(1082392, 1000),	// Lv120 - 英雄手套 - 送给武陵道场最强挑战者的传说中的手套。
						new Array(1132115, 800),	// Lv120 - 武公的黑腰带(无描述)
						new Array(2046856, 500),		// 专属饰品攻击力卷轴 - 在饰品上增加物理攻击力属性。
						new Array(2046857, 500)		// 专属饰品魔力卷轴 - 在饰品上增加魔法攻击力属性。
						);
...
var secretBox = 2430069; // 开心转盘箱子
var rewardsel;
var freetimes = 2;		// 免费次数
var basemeso = 10000000;	// 起步收费
var needmeso = 0;
...
function action(mode, type, selection) {
	if (mode <= 0) {
		...
		if (status == 0) {
			...
			text = "\t\t\t\t#r#e- 新版武陵道场 -#n#k\r\n\r\n\t欢迎来到新版武陵道场，这里每一层都有实力强大的怪物与你一决雌雄，当然奖励也很丰富，运气好的话可以从怪物身上获得#b#z" + taskItem + "##k，可在我这里兑换特有的装备奖励，随着难度的提升，经验也会越来越丰富，目前已开放#r30层#k，且在#r17层、29层#k消灭怪物后会获得#b<#z" + secretBox + "#>#k丰富大奖，要注意捡取哦！期待您的精彩表现~\r\n\r\n";
			text += "#d今日已挑战的次数：#e" + times + "#n\r\n再次挑战需要金币：#e" + needmeso + "#n#k\r\n";//您的最佳成绩：0层\r\n
			text += "#b#L1#新版武陵道场说明#l  #L2#兑换奖励#l   #L0#开始挑战！#l\r\n\t";
			//text += "#b#L2#兑换奖励#l   #L0#开始挑战！#l\r\n";
			cm.sendSimple(text);
		} else if (status == 1) {
			firstsel = selection;
			if (selection == 0) {
				...
				text += "- 通关奖励\r\n  每层都会有机率掉落#b#z" + taskItem + "##k，在#r17层、29层#k消灭该层所有怪物后会掉落#b<#z" + secretBox + "#开心礼物>#k，打开后可获得数量不等的奖励。\r\n";
				text += "- 每日挑战次数\r\n  每日可获得#r2次#k免费挑战次数，使用完后再次挑战则需消耗金币才能继续挑战，当日挑战次数越多，所需金币也会越多。\r\n";
				cm.sendNext(text);
				status = -1;
				return;
			} else if (selection == 2) {
			...
			if (firstsel == 0) {
				...
				cm.worldMessageYellow("[系统公告] : 玩家[" + cm.getPlayer().getName() + "]进入了新版武陵副本，祝TA好运~");
				cm.dispose();
			} else if (firstsel == 2) {
