...
function action(mode, type, selection) {
	if (mode == -1) {
		...
		if (mode == 1)
			...
			status--;
		if (status == 0) { 
			cm.sendSimple("亲爱的#r#h ##k您好，我是会员八折伤害上限突破系统:\r\n#b#L1#伤害上限突破简介#l\r\n\r\n#L3##b[武器]使用中介币破功#k#b (每次追加上限：#r100W#b)#l");
		} else if (status == 1) {
			if (selection == 1) {
			...
			if(cm.haveItem(4001485, 120)){
			if (cm.changeLimitBreak(1000000)) {
			    cm.setBossLog("点卷突破1",1);
			    cm.gainItem(4001485,-120);
    			    cm.sendOk("#b伤害上限突破成功.\r\n\r\n本次追加伤害为：#r1000000#b.");
			    cm.worldSpouseMessage(0x20,"[会员八折破功] 会员玩家 "+ cm.getChar().getName() +" 在随身服务中使用 中介币 让武器伤害上限突破成功 本次追加 100W 伤害值。");
			}else{
    			    cm.sendOk("#b突破失败.\r\n系统为检测到角色身上装备武器.");
			}
    			    cm.dispose();
