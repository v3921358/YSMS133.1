...
function action(mode, type, selection) {
	if (mode == -1) {
		...
		if (status == 0) {
			...
			ca.set(2015, 8 - 1, 1, 20, 10, 0); //设置开放时间 月份要-1才是当前月份
			var startTimestamp = ca.getTimeInMillis();
			if (currentTimestamp < startTimestamp) {
				var lasttime = (startTimestamp - currentTimestamp) / (60 * 1000);
				cm.sendOk("限量礼包将于今晚" + ca.get(java.util.Calendar.HOUR_OF_DAY) + ":" + ca.get(java.util.Calendar.MINUTE) + "开始发放，本次发放#r30#k个礼包，距离活动开始还有#r" + Math.floor(lasttime) + "#k分钟，抓紧时间把包裹整理干净吧！！请各位做好准备！");
				cm.dispose();
				return;
			}
