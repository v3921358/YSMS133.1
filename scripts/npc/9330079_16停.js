...
function action(mode, type, selection) {
    ...
    if (status == 0) {
		...
		selStr += "#d您当前的抵用卷为：#r" + cm.getPlayer().getCSPoints(2) + " #d点\r\n(市场可以泡点哦,每30秒获得一次奖励).#k\r\n\r\n";
		selStr += "#r#L1#"+ aaa +" 10000抵用卷换取#t5050000#x100#l\r\n";
		selStr += "#r#L0#"+ aaa +" 300抵用卷换取#t5062000#x1( "+cm.getBossLog("抵用卷神奇")+" / 30次)#l\r\n";
		selStr += "#L2#"+ aaa +" 2000抵用卷随即抽取140高级装备#l\r\n";
		selStr += "#L4#"+ aaa +" 1000抵用卷换取#t5062002#x1( "+cm.getBossLog("抵用卷魔方")+" / 30次)#l\r\n";
		//selStr += "#L6#"+ aaa +" 1000抵用卷换取#t2290884#x1( "+cm.getBossLog("双弩神秘手册")+" / 10次)#l\r\n";
		selStr += "#L5#"+ aaa +" 2000抵用卷换取#t2340000#( "+cm.getBossLog("抵用卷祝福")+" / 30次)#l\r\n";
		//selStr += "#L7#"+ aaa +" 1亿金币换取2000抵用卷#l\r\n";
		//selStr += "#L3#"+ aaa +" 300000绑定奇幻国度币抽取极真系列#l\r\n";
 cm.sendSimple(selStr);
    } else if (status == 1) {
        switch (selection) {
        case 0:
            if (cm.getPlayer().getCSPoints(2) >= 300 && cm.getBossLog("抵用卷神奇") <= 29){
                cm.gainNX(2, -300);
		...
		cm.dispose();
            ...
            break;
        case 2:
            if (cm.getPlayer().getCSPoints(2) >= 2000){
                cm.gainNX(2, -2000);
		cm.gainItem(item[chance],1);
                cm.sendOk(head + "兑换成功。抵用卷减2000,获取#z"+item[chance]+"一个.");
		cm.worldSpouseMessage(0x20, "『抵用卷兑换』" + " : " + "恭喜" + cm.getPlayer().getName() + ",在抵用商店处用2000抵用卷抽取 " + cm.getItemName(item[chance]) + " 一个。");
		cm.dispose();
            } else {
                cm.sendOk(head + "#r兑换失败:\r\n\r\n#b1). 你没有足够的抵用卷(请在市场挂机,或打怪获得)!");
				cm.dispose();
		...
		cm.worldSpouseMessage(0x20, "『绑定奇幻国度币兑换』" + " : " + "恭喜" + cm.getPlayer().getName() + ",在抵用商店处用1500绑定奇幻国度币抽取 " + cm.getItemName(jizhen[chance]) + " 一个。");
		cm.dispose();
            } else {
            ...
            break;
        case 4:
            if (cm.getPlayer().getCSPoints(2) >= 1000 && cm.getBossLog("抵用卷魔方") <= 29){
                cm.gainNX(2, -1000);
		...
		cm.dispose();
            ...
            break;
        case 5:
            if (cm.getPlayer().getCSPoints(2) >= 2000 && cm.getBossLog("抵用卷祝福") <= 29){
                cm.gainNX(2, -2000);
		...
		cm.worldSpouseMessage(0x20, "『抵用卷兑换』" + " : " + "恭喜" + cm.getPlayer().getName() + ",在抵用商店处用用一亿金币兑换2000抵用卷。");
		cm.dispose();
            } else {
                cm.sendOk(head + "#r兑换失败:\r\n\r\n#b1). 金币不足。");
				cm.dispose();
