            ...
            if (selection == 0) {
                ...
                if (cm.getParty() == null) { // 没有组队
					...
					if (getBossLog(idx)>=maxPlay) {
						cm.sendOk("队伍中有玩家已经参与过该副本3次，无法再进入，请踢出该玩家。");
						cm.dispose();
						return;
					}
                ...
                cm.sendOk("副本阿卡伊勒，如果火力不足够，是会自动脱离战场的。击败最后就能开启#e#b神秘宝箱#n#k，里面有传说中的#b法弗纳武器制作材料#k，还有各种新奇的椅子和时装；开启宝箱后，玩家通过掷点的方式来赢取奖励。掷点时，你需要在10秒钟内做出需求选择，否则将会被强制下线。\r\n进入副本前，请确认你的组队员：\r\n\r\n#b1、组队员必须要" + minPartySize + "人以上，" + maxPartySize + "人以下。\r\n2、组队员等级必须要在" + minLevel + "级以上。\r\n\r\n（#r如果仍然错误, 重新下线,再登陆 或者请重新组队。#k#b）");
                cm.dispose();
            } else if (selection == 2) {
                cm.warp(910000000, 0)
                cm.sendOk("我等着更加强大的你……")
...
function getBossLog(idx) {
	...
	var sql = "SELECT max(count) as maxcount FROM bosslog where bossid = '阿卡伊勒' and characterid in (" + idStr + ") and to_days(time) = to_days(now());";
