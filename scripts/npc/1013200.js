	...
	if (mode == -1) {
		...
		if (status == 0) {
			...
			if(hour == 20 && (minute >= 1 && minute <= 2) && (weekday == 6 || weekday == 5 || weekday == 0|| weekday == 4 || weekday == 3|| weekday == 2 || weekday == 1)){// || cm.getPlayer().getName() == "管理员哈士奇"){
				...
				if (random >= 800 && random <= 810) {
				...
				} else if (random >= 1000 && random <= 1150) {
				...
				} else if (random >= 1300 && random <= 1400) {
				
					cm.getMap().startMapEffect("恭喜玩家 "+cm.getChar().getName()+" 额外获得【彩虹枫叶】五个。", 5120012);
					...
					cm.getMap().startMapEffect("恭喜玩家 "+cm.getChar().getName()+" 额外获得【爱心宝石】十个。", 5120012);
					cm.worldSpouseMessage(0x15, "[疯狂鼠标] : 恭喜 【" + cm.getChar().getName() + "】 在狂点鼠标的时候额外获得【爱心宝石】10个.");
					cm.gainItem(4001465, 10);


				} else if (random >= 2000 && random <= 2500) {
				...
				} else if (random >= 3600 && random <= 3700) {
