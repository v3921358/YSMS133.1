...
var packname = new Array(
        Array("高级神奇魔方礼包", 400000),
        Array("升级强化型大礼包", 400000),
        Array("惊人正义卷大礼包", 400000),
        Array("附加潜能卷大礼包", 400000)
        );
...
function action(mode, type, selection) {
    if (mode == -1) {
        ...
        if (status == 0) {
            ...
            selStr += "- #e#d请选择需要购买的礼包#n\r\n";
            for (var i in packname) {
                selStr += "#L" + i + "##b" + aaa + " 购买 #r" + packname[i][0] + "#k #b需要 #r" + packname[i][1] + " #k#b抵用券#l\r\n";
            }
            selStr += " \r\n\r\n";
        	...
        	cm.sendYesNo(head + "确定购买 #r" + packname[typed][0] + "#k 吗? 将会使用掉您 #r" + packname[typed][1] + "#k 抵用券，使用后将获得\r\n\r\n" + selStr);
        } else if (status == 2) {
			var nx1 = cm.getNX(1);
			var nx2 = cm.getNX(2);
			if (nx1 < packname[typed][1] && nx2 < packname[typed][1] || !checkpack()) {
                ...
                cm.worldSpouseMessage(0x20, "『抵用券商城』 : 恭喜 " + cm.getChar().getName() + " 用抵用券购买" + packname[typed][0] + "一个.");
                cm.dispose();
			}
        }
    }
