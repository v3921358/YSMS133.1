...
var chance1 = Math.floor(Math.random() * 50 + 50);
var chance2 = Math.floor(Math.random() * 100 + 900);
var chance3 = Math.floor(Math.random() * 4 + 1);
var chance4 = Math.floor(Math.random() * 10 + 20);
...
var hwtext=new Array("有种放学别走....和本姑娘单挑","我爸是李嘉诚,你想怎么着","看什么看,没见过美女吗？","怎么看你都像2B，不服就来单挑我呀""就你这骨瘦如柴的皮包骨还想单挑吗？","2B,就你这傻样还想单挑本姑娘吗？","怕死的赶快给本姑娘求饶,傻B");
...
if (selection == 1) {
	...
	cm.worldSpouseMessage(0x24, "调教美女 : " + cm.getChar().getName() + " 胜利获得 ["+ii.getName(item[chance])+"] 一个.");
	cm.dispose();
	} else if (xxx == 3) {
	cm.gainNX(1, chance1);
	cm.sendOk(""+win+"\r\n\r\n- #e#d您不费吹灰之力已将美女打趴下。#n\r\n\r\n- #e#d本次获得点卷：#n#r"+chance1+"#k");
	cm.worldSpouseMessage(0x24, "调教美女 : " + cm.getChar().getName() + " 胜利获得 "+chance1+" 点卷奖励.");
	cm.dispose();
	} else if (xxx == 5) {
	cm.gainNX(2, chance2);
	cm.sendOk(""+win+"\r\n\r\n- #e#d您不费吹灰之力已将美女打趴下。#n\r\n\r\n- #e#d本次获得抵用卷：#n#r"+chance2+"#k");
	cm.worldSpouseMessage(0x24, "调教美女 : " + cm.getChar().getName() + " 胜利获得 "+chance2+" 抵用卷奖励.");
	cm.dispose();
	} else if (xxx == 7) {
	cm.gainItem(laba[chance], chance3);
	cm.sendOk(""+draw+"\r\n\r\n- #e#d战成平手。什么个情况？#n\r\n\r\n- #e#d本次获得：#n#r#z"+laba[chance]+"# x "+chance3+"#k");
	cm.worldSpouseMessage(0x24, "调教美女 : " + cm.getChar().getName() + " 战成平手获得 ["+ii.getName(laba[chance])+"] x "+chance3+".");
	cm.dispose();
	} else if (xxx == 9) {
	cm.gainItem(etc[chance], chance4);
	cm.sendOk(""+draw+"\r\n\r\n- #e#d战成平手。什么个情况？#n\r\n\r\n- #e#d本次获得：#n#r#z"+etc[chance]+"# x "+chance4+"#k");
	cm.worldSpouseMessage(0x24, "调教美女 : " + cm.getChar().getName() + " 战成平手获得 ["+ii.getName(etc[chance])+"] x "+chance4+".");
	cm.dispose();
	} else {
	cm.sendOk(""+lose+"\r\n\r\n- #e#d您被美女调教的鼻青脸肿，真丢脸，请再次尝试。#n\r\n\r\n");
	cm.dispose();
	}
    if(typed==2){
	cm.dispose();
