...
function action(mode, type, selection) {
    if (mode == -1) {
        ...
        if (status == 0) {
            ...
            add += "" + xxx + "-当前下注押金:#b<#e#r 冒险币赌博 #n#b>#b<#e#r " + cost + " 冒险币#n#b >#k\r\n "; //cm.getPlayer().player.GetMoney()
            add += "" + xxx + "-当前您拥有冒险币：" + cm.getMeso() + ".#k\r\n";
            add += "#L0#" + ttt + "-[#r加注#k]#l\r\n\r\n";
            ...
            if (selection == 0) {
                cm.sendOk("#b成功加注#r2冒险币#b,请点确定后查看.");
                cost = cost + 1000000;
                status = -1;
            } else if (selection == 1) {
                var add = "#b<#e#r 冒险币赌博 #n#b>\r\n\r\n";
			...
			//jiaru = GetRandomNum(0, jilv);
                        ...
                        if (jiaru == 2) {
                            costa = cost * jilv
                            cm.gainMeso(cost);
                            //cm.getC().getChannelServer().broadcastPacket(Packages.tools.MaplePacketCreator.serverNotice(0x19, cm.getC().getChannel(), "[博彩公告]" + " : 恭喜玩家：" + cm.getPlayer().getName() + " 在冒险币博彩中赢得" + costa + "冒险币。"));
