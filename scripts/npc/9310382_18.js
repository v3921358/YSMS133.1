...
function action(mode, type, selection) { 
	...
	if(cm.getText() < 1){
	    ...
	    cm.sendYesNo("您好,欢迎使用点卷增加蓝量.\r\n增加#r" + cm.getText() + "#k蓝量将会使用掉您#r" + cm.getText() * 3 + "#k点卷\r\n请确认后使用。"); 
	    } 
        } else if (status == 2) { 
		var getmaxmp = cm.getChar().getStat().getMaxMp();
	if (cm.getPlayer().getCSPoints(1) >= cm.getText() * 3) { 
		   cm.gainNX(-cm.getText() * 3);
		   cm.getChar().getStat().setMaxMp(getmaxmp+cm.getText() * 1,cm.getChar());
           cm.worldSpouseMessage(0x24,"[增加MP上限] ：恭喜玩家 "+ cm.getChar().getName() +" 在随身NPC中用 "+ cm.getText() * 3 +" 点卷增加了 "+ cm.getText() +" MP上限");
