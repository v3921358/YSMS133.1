        ...
        if (status == 0) {
            ...
            if (selection == 0) {
                if (cm.getParty() == null) { // 没有组队
                    ...
                    cm.dispose();
                   /* } else if (cm.getBossLog('ZChaosPQ10') >= 3){
                     cm.sendOk("对不起，一天只能进入3次。")
                     cm.dispose();*/
                    } else  {
		//if (cm.getEventCount("皇陵1") < 3){
		if (cm.checkPartyEventCount("皇陵,3")){//判断队员是否都小于这个次数
                    ...
                    if (next) {
                        var em = cm.getEventManager("ZChaosPQ10");
                        if (em == null) {
                            cm.sendOk("此任务正在建设当中。");
                        } else {
				...
				cm.setPartyEventCount("皇陵");//给所有队员次数都+1
                                cm.dispose();
                                return;
                            } else {
                                cm.sendOk("[日常]扫荡秦皇陵任务里面已经有人了，请稍等！");
                            }
                        }
                        cm.dispose();
                    } else {
			...
			//}
        		...
        		cm.dispose();
        		cm.sendOk("只有在1,2,3频道才可以参加[日常]扫荡秦皇陵任务。");
	}
    }
}