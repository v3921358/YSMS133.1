...
function action(mode, type, selection) { 
	...
	if(cm.getText() < 1){
	    ...
	    cm.sendYesNo("您好,欢迎使用抵用卷增加血量.\r\n增加#r" + cm.getText() + "#k血量将会使用掉您#r" + cm.getText() * cost + "#k点抵用卷\r\n请确认后使用。"); 
	    } 
        } else if (status == 2) { 
	...
	if (cm.getPlayer().getCSPoints(1) >= cm.getText()*cost) { 
           ...
           cm.worldSpouseMessage(0x20,"[增加HP上限] ：恭喜玩家 "+ cm.getChar().getName() +" 在随身NPC中用 "+ cm.getText() * cost +" 抵用卷增加了 "+ cm.getText() +" HP上限");
