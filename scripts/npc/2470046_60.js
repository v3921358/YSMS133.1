...
function action(mode, type, selection) {
    ...
    if (status == 0) {
        ...
        cm.sendOk("生成红包数据成功！！");
		cm.worldMessageEffect("[红包福利] " + " : " + "哇塞，拼手速抢红包，在市场7洞门口NPC【烟花商人】处开启了，大家快来抢红包 " , 16, 30);

        cm.worldSpouseMessage(0x23, "『红包福利』" + " : " + "哇塞， " + cm.getChar().getName() + " 开启了限量抢红包活动。赶紧抓紧时间抢吧。");
        cm.worldSpouseMessage(0x23, "『红包福利』" + " : " + "哇塞， " + cm.getChar().getName() + " 开启了限量抢红包活动。赶紧抓紧时间抢吧。");
        cm.worldSpouseMessage(0x23, "『红包福利』" + " : " + "哇塞， " + cm.getChar().getName() + " 开启了限量抢红包活动。赶紧抓紧时间抢吧。");
        cm.worldSpouseMessage(0x23, "『红包福利』" + " : " + "哇塞， " + cm.getChar().getName() + " 开启了限量抢红包活动。赶紧抓紧时间抢吧。");
        cm.worldSpouseMessage(0x23, "『红包福利』" + " : " + "哇塞， " + cm.getChar().getName() + " 开启了限量抢红包活动。赶紧抓紧时间抢吧。");
    ...
    while (getMoney.next()) {
        switch (parseInt(getMoney.getString("luckmoneytype"))) {
            case 0://金币
                ...
                cm.sendOk("领取成功啦！！！");
				cm.worldMessageEffect("『拼手速抢红包』" + " : " + "玩家 " + cm.getChar().getName() + " 领取了管理员发放的 " + parseInt(getMoney.getString("LuckyMoney")) + " 金币红包 " , 1, 10);

                cm.worldSpouseMessage(0x23, "『拼手抢红包』" + " : " + "玩家 " + cm.getChar().getName() + " 领取了管理员发放的 " + parseInt(getMoney.getString("LuckyMoney")) + " 金币红包。");
                break;
            case 1://点券
                cm.gainNX(1, parseInt(getMoney.getString("LuckyMoney")));
                cm.sendOk("领取成功啦！！！");
				cm.worldMessageEffect("『拼手速抢红包』" + " : " + "玩家 " + cm.getChar().getName() + " 领取了管理员发放的 " + parseInt(getMoney.getString("LuckyMoney")) + " 点卷红包 " , 1, 10);

                cm.worldSpouseMessage(0x23, "『拼手速抢红包』" + " : " + "玩家 " + cm.getChar().getName() + " 领取了管理员发放的 " + parseInt(getMoney.getString("LuckyMoney")) + " 点卷红包。");
                break;
            case 2://抵用
                cm.gainNX(2, parseInt(getMoney.getString("LuckyMoney")));
                cm.sendOk("领取成功啦！！！");
				cm.worldMessageEffect("『拼手速抢红包』" + " : " + "玩家 " + cm.getChar().getName() + " 领取了管理员发放的 " + parseInt(getMoney.getString("LuckyMoney")) + " 抵用卷红包 " , 1, 10);

                cm.worldSpouseMessage(0x23, "『拼手速抢红包』" + " : " + "玩家 " + cm.getChar().getName() + " 领取了管理员发放的 " + parseInt(getMoney.getString("LuckyMoney")) + " 抵用卷红包。");
                break;
            case 3://现金

                cm.gainRMB(parseInt(getMoney.getString("LuckyMoney")));
                cm.sendOk("领取成功啦！！！");
				cm.worldMessageEffect("『拼手速抢红包』" + " : " + "玩家 " + cm.getChar().getName() + " 领取了管理员发放的 " + parseInt(getMoney.getString("LuckyMoney")) + " 现金红包 " , 1, 10);

                cm.worldSpouseMessage(0x23, "『拼手速抢红包』" + " : " + "玩家 " + cm.getChar().getName() + " 领取了管理员发放的 " + parseInt(getMoney.getString("LuckyMoney")) + " 现金红包。");
                break;
            case 4://道具
                cm.gainItem(parseInt(getMoney.getString("LuckyMoney")), 1);
                cm.sendOk("领取成功啦！！！");
				cm.worldMessageEffect("『拼手速抢红包』" + " : " + "玩家 " + cm.getChar().getName() + " 领取了管理员发放的 " + parseInt(getMoney.getString("LuckyMoney")) + " 道具红包 " , 1, 10);

                cm.worldSpouseMessage(0x23, "『拼手速抢红包』" + " : " + "玩家 " + cm.getChar().getName() + " 领取了管理员发放的 " + ii.getName(parseInt(getMoney.getString("LuckyMoney"))) + " 道具红包。");
                break;
        }
