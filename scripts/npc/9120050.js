        ...
        if (cm.getPlayer().getClient().getChannel() == 1 || cm.getPlayer().getClient().getChannel() == 2 || cm.getPlayer().getClient().getChannel() == 3) {
            if (status == 0) {
                if (cm.getMap().getId() == 802000821) {
                    ...
                    cm.sendSimple("- #e#d拥抱罗玉凤#k#n:\r\n\r\n#b阿利博士实行生化魔人改造试验失败了，因此生化魔人绑架了#r#p9310114##b，但是由于生化魔人的威力太过强大，需要年轻的冒险家们进行解救。成功解救后将获得罗玉凤赠送心仪的礼物一个。。#k\r\n副本要求：\r\n#r1). 可在1,2,3线可挑战。\r\n2). 组队员等级必须要在" + minLevel + "级以上。\r\n3). 组队员必须要" + minPartySize + "人以上，" + maxPartySize + "人以下。\r\n\r\n#L0#[执行]拥抱罗玉凤#l")
                }
            } else if (status == 1) {
                if (selection == 0) {
                    if (cm.getParty() == null) { // 没有组队
                        ...
                        cm.sendOk("本次拥抱罗玉凤副本已经在进行中。请等待或者换线后尝试..");
                        cm.dispose();
                    } else {
                        if (cm.getBossLog("拥抱罗玉凤") < 2) {
                            //if (cm.checkPartyEventCount("罗玉凤1")){
                            ...
                            if (next) {
                                ...
                                if (em == null) {
                                    ...
                                    if (prop.equals("0") || prop == null) {
                                        ...
                                        cm.setBossLog("拥抱罗玉凤");
                                        cm.worldSpouseMessage(0x20, "[拥抱罗玉凤] ：玩家 " + cm.getChar().getName() + " 进入生化魔人 欧啦啦 栖息之地 <无引力实验室>，请小心。");
                                        cm.dispose();
                                        return;
                                    } else {
