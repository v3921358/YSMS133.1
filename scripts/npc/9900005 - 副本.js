...
function action(mode, type, selection) {
	if (mode == -1) {
		...
		if (mode == 1)
			...
			status--;
		if (status == 0) { 
			cm.sendSimple("#fUI/UIWindow2.img/QuestAlarm/BtQ/normal/0#亲爱的 #r#h ##k 您好，我是新手成长系统:\r\n#k#fUI/UIWindow2.img/QuestAlarm/BtQ/normal/0#当前糖果：#r"+cm.getPlayerPoints()+"#k 点     #fUI/UIWindow2.img/QuestAlarm/BtQ/normal/0#今天在线：#r"+cm.getGamePoints()+"#k 分钟#b\r\n\r\n#fUI/UIWindow2.img/QuestGuide/Button/WorldMapQuestToggle/normal/0#\r\n#k#L1##fUI/UIWindow2.img/QuestAlarm/BtQ/normal/0##r[必须] #b新手成长系统简介#l\r\n#k#L4##fUI/UIWindow2.img/QuestAlarm/BtQ/normal/0##r[必须]#b 阅读新手问题指导#l\r\n#L3##fUI/UIWindow2.img/QuestAlarm/BtQ/normal/0##r[必须]#b 阅读玩法指导内容#l\r\n#L5##fUI/UIWindow2.img/QuestAlarm/BtQ/normal/0##r[必须]#b 领取成长等级奖励#l\r\n\t\t");
		} else if (status == 1) {
			if (selection == 1) {
			cm.sendOk("#fUI/UIWindow2.img/Quest/quest_info/summary_icon/summary#\r\n#fUI/UIWindow2.img/QuestAlarm/BtQ/normal/0#亲爱的#r#h ##k您好,我是新手成长系统简介:\r\n  职业: 所有\r\n  等级: 30 60 100 150 180 210 230 250\r\n  推荐升级地图: 系统引导地图练级\r\n  引导使用某道具: 系统引导使用某道具\r\n  赠送道具: 当前职业对应等级防具,武器\r\n#fUI/UIWindow2.img/Quest/quest_info/summary_icon/startcondition#\r\n#fUI/UIWindow2.img/QuestAlarm/BtQ/normal/0#该角色达到等级要求即可完成1次阶段功能.\r\n\r\n#fUI/UIWindow2.img/QuestAlarm/BtQ/normal/0##r注：角色不能超过等级范围或必须与等级对应转职数.\r\n#fUI/UIWindow2.img/QuestAlarm/BtQ/normal/0##r注：装备 消耗 设置 其它 特殊 背包栏 预留 10 格以上.\r\n#fUI/UIWindow2.img/QuestAlarm/BtQ/normal/0##r注：若达到等级未达到转职数，背包满了,无法领取武器(后果自负).");
                    	cm.dispose();
			} else if (selection == 2) {
			if((cm.getPlayer().getLevel() > 9 && cm.getPlayer().getLevel() < 30) && cm.getBossLog("引导10",1) == 0 && (cm.getSpace(1) > 5||cm.getSpace(2) > 5||cm.getSpace(3) > 5||cm.getSpace(4) > 5)){
				...
				cm.gainItem(3700012,1,30);
           			cm.worldSpouseMessage(0x20,"[新手系统] 玩家 "+ cm.getChar().getName() +" 完成了芒果新手成长系统阶段1功能 系统给予大量奖励。");
				cm.sendOk("完成了芒果新手成长系统阶段功能 系统给予大量奖励。");
				cm.dispose();
				...
				cm.gainItem(2001505,100); // 超级药水
           			cm.worldSpouseMessage(0x20,"[新手系统] 玩家 "+ cm.getChar().getName() +" 完成了芒果新手成长系统阶段2功能 系统给予大量奖励。");
				cm.sendOk("完成了芒果新手成长系统阶段功能 系统给予大量奖励。");
				cm.dispose();
				...
				cm.gainItem(2022956,10); // 火红玫瑰
				cm.gainItem(2001505,100); // 超级药水
           			cm.worldSpouseMessage(0x20,"[新手系统] 玩家 "+ cm.getChar().getName() +" 完成了芒果新手成长系统阶段3功能 系统给予大量奖励。");
				cm.sendOk("完成了芒果新手成长系统阶段功能 系统给予大量奖励。");
				cm.dispose();
				...
				cm.gainItem(2022956,10); // 火红玫瑰
				cm.gainItem(2001505,100); // 超级药水
           			cm.worldSpouseMessage(0x20,"[新手系统] 玩家 "+ cm.getChar().getName() +" 完成了芒果新手成长系统阶段4功能 系统给予大量奖励。");
				cm.sendOk("完成了芒果新手成长系统阶段功能 系统给予大量奖励。");
				cm.dispose();
				...
				cm.gainItem(2001505,100); // 超级药水
           			cm.worldSpouseMessage(0x20,"[新手系统] 玩家 "+ cm.getChar().getName() +" 完成了芒果新手成长系统阶段5功能 系统给予大量奖励。");
				cm.sendOk("完成了芒果新手成长系统阶段功能 系统给予大量奖励。");
				cm.dispose();
			}else{
		...
		} else if (selection == 5) {//等级奖励
			cm.dispose();
			cm.openNpc(9900005,1002);
		} else if (selection == 6) {//新手问答与奖励
			cm.dispose();
