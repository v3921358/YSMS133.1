...
var itemList = Array(
Array(5360015, 3, 1), //双倍爆率卡一天权
//Array(5211060, 10, 2 * 60 * 60 * 1000), //三倍经验卡(2小时) 
Array(5210004, 3, 1), //双倍经验值卡一天权(晚)
Array(5210002, 3, 1)  //双倍经验值卡一天权(白)
);
var itemId = -1;
var points = -1;
var period = -1;
...
function action(mode, type, selection) {
    ...
    if (status == 0) {
        ...
        if (item != null) {
            ...
            points = item[1];
            period = item[2];
            cm.sendYesNo("您是否购买#i" + itemId + ":# #b#t" + itemId + "##k 需要 #r" + points + "#k 余额？");
        } else {
            cm.sendOk("出现错误...");
            cm.dispose();
        }
	...
	if (cm.getRMB() >= points) {
            ...
            cm.sendOk("您没有那么多余额。\r\n\r\n购买#i" + itemId + ":# #b#t" + itemId + "##k 需要 #r" + points + "#k 余额。");
        }
        cm.dispose();
    }
}