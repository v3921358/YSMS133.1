    ...
    if (mode == -1) {
        ...
        if (status == 0) {
            ...
            for (var i in itemlist) {
                selStr += "#L" + i + "##b" + aaa + "购买" + itemlist[i][0] + " #r#z" + itemlist[i][1] + "##b 需" + (i == 9 || i == 11 ? "" : "要") + itemlist[i][2] + "#k #b抵用卷#l\r\n";
            }
            selStr += " \r\n\r\n";
            cm.sendSimple(selStr);
        } else if (status == 1) {
			seld = selection;
			cm.sendYesNo(head + "确定购买 #r#z" + itemlist[seld][1] + "##k 将会使用掉您 #r" + itemlist[seld][2] + "抵用券. 您将获得#r" + itemlist[seld][3] + "#k" + itemlist[seld][0] + "#z" + itemlist[seld][1] + "#。");
        } else if (status == 2) {
			var nx1 = cm.getNX(2);
			var nx2 = cm.getNX(2);
			if (nx1 < itemlist[seld][2] && nx2 < itemlist[seld][2] || cm.getSpace(1) < 1) {
				...
				cm.worldSpouseMessage(0x20, "『抵用卷商城』 : 恭喜 " + cm.getChar().getName() + " 用抵用卷购买" + cm.getItemName(itemlist[seld][1]) + "一个.");
			}
			cm.dispose();
        }
    }
