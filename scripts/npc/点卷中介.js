...
function action(mode, type, selection) {
	...
	if (status == 0) {
		...
		if (selection == 3) {
			status = -1;
			cm.sendSimple("1个#b#t"+transId+"##k可以兑换#r900点卷#k，#r1000点卷#k可以兑换1个#b#t"+transId+"##k。#b#t"+transId+"##k可用于玩家之间的交易与贩售。");
		} else if (selection == 1) {
			var maxTimes = cm.getItemQuantity(transId);
			cm.sendGetNumber("#d#e<新年元宝兑换点卷>#n#k\r\n当前最多可以兑换#r"+(maxTimes*900)+"#k点卷，最多输入#r"+maxTimes+"#k。\r\n请输入兑换的#b#t"+transId+"##k数量:\r\n兑换比例为 1 : 900\r\n", 1, 1, maxTimes);
		} else if (selection == 2) {
			var maxTimes = Math.floor(cm.getPlayer().getCSPoints(1)/1000);
			if (maxTimes>300)
				maxTimes = 300;
			cm.sendGetNumber("#d#e<点卷兑换新年元宝>#n#k\r\n本次最多可以兑换#r"+maxTimes+"#k个#b#t"+transId+"##k\r\n请输入兑换的#b#t"+transId+"##k数量:\r\n兑换比例为 1000 : 1\r\n", 1, 1, maxTimes);
		}
	} else if (status == 2) {
		...
		if (typed == 1) {
			...
			if (cm.getSpace(4)<1) {
				...
				if (cm.getPlayer().getCSPoints(1)>=maxNumber) {
					...
					cm.gainItem(transId, quantity);
					cm.gainNX(-maxNumber);
					cm.sendSimple("成功兑换了#r"+quantity+"#k个新年元宝");
				} else {
