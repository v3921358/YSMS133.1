...
function action(mode, type, selection) { 
    if (mode == -1) { 
        ...
        if (status == 0) { 
	    abb = 1;
	    cm.sendGetText("您好,欢迎使用点卷#e#r增加HP#n#k.每10点卷增加1点最大#e#rHP值#k#n。\r\n\r\n#r嫌贵请使用智力点洗血，或者办理理财\r\n\r\n#b温馨提示：办理理财随身服务洗血只需5折#k\r\n\r\n目前点卷：#r"+cm.getPlayer().getCSPoints(1)+" #k点\r\n\r\n#r注：请输入..最大HP不能超过50万,否则出错该不负责");
        } else if (status == 1) { 
	if(cm.getText() < 1){
	    ...
	    cm.sendYesNo("您好,欢迎使用点卷增加血量.\r\n增加#r" + cm.getText() + "#k血量将会使用掉您#r" + cm.getText() * 10 + "#k点卷\r\n请确认后使用。"); 
	    } 
        } else if (status == 2) { 
	...
	if (cm.getPlayer().getCSPoints(1) >= cm.getText() * 10) { 
           ...
           cm.worldSpouseMessage(0x24,"[增加HP上限] ：恭喜玩家 "+ cm.getChar().getName() +" 在拍卖更多服务处用 "+ cm.getText() * 10 +" 点卷增加了 "+ cm.getText() +" HP上限");
