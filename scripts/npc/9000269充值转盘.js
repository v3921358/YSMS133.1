...
function action(mode, type, selection) {
	...
	if (status == 0) {
		if ((cm.getSpace(1) > 1 || cm.getSpace(2) > 1 || cm.getSpace(3) > 1 || im.getSpace(4) > 1)) {
			cm.sendYesNo("这里是现金抽奖专区。需要赞助人民币 [50元] 才能抽奖一次,我背包里面包含了必成 宿命/星火 卷轴、最高贝首饰，大PB椅子、大西瓜，大太阳，青蛙跳楼机， 七彩摩天轮，十二星座椅子等， 更有V125最新版椅子,140/150FFN 武器  110 /150极真防具， 160 漩涡套装等。我这里道具样样是精品,奖品特别丰富哦快来抽奖吧! 每赞助  [50元]    可以抽奖一次,   你确定要抽奖吗？");
		} else {
			cm.sendOk("我凑,你想坑死我呀,你能不能不要坑爹了,您背包空间不足,或者您没有赞助芊芊美女  [50元]  ,   请整理您的背后或赞助后再来点我");
			cm.safeDispose();
		}
	} else if (status == 1) {
		var points = getMyPoints();
		if (points<50) {
			cm.sendOk("我草,你坑爹呀,不带你这么坑爹的吧,您当前充值积分已经小于  50元  ,请找客服MM赞助后再来抽奖吧,");
			cm.dispose();
			return;
		}
		gainPoint(-50);
		if ((cm.getSpace(1) > 1 || cm.getSpace(2) > 1 || cm.getSpace(3) > 1 || im
				.getSpace(4) > 1)) {
			var item = -1;
			...
			if (chance >= 0 && chance <= 6) {
				...
				cm.worldMessage("恭喜玩家" + cm.getChar().getName()
						+ "牛气冲天,在现金冲值转盘中吸取20E经验。请再来一次,终极大奖等你来领取");
				cm.dispose();
			} else if (chance >= 7 && chance <= 10) {
				cm.gainMeso(100000000);
				text = "恭喜你牛气冲天获得了1E金币。请再来一次,超级大奖等你来领取";
				cm.worldMessage("恭喜玩家" + cm.getChar().getName()
						+ "牛气冲天,在现金充值积分转盘中无意拿到1E支票。请再来一次,超级大奖等你来领取");
				cm.dispose();
			} else {
