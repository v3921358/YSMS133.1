...
function playerEntry(eim, player) {
    ...
    EventDataBase = DatabaseConnection.getConnection().prepareStatement("SELECT times FROM limitlessEvent where charid = " + charid + "").executeQuery();
...
function SpwnMobForPlayer(eim) {
    if (GiftTimes != 0) {
        ...
        eim.broadcastPlayerMsg(-1, "[无限战斗] 现在是第" + (Times + 1) + "关！ 每通关10层后可获得大量道具奖励! ");
    }
...
function allMonstersDead(eim) {
    var UpDateData = DatabaseConnection.getConnection().prepareStatement("update limitlessEvent set times=? where charid = " + charid + "")
    ...
    em.schedule("SpwnMobForPlayer", 1000 * 8, eim);//10秒后传送
}
