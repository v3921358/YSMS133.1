...
public class DumpBuffInformation {
    ...
    public static void main(String[] args) {
        ...
        System.out.println("是否导出技能信息？ <y/n>");
        boolean export = input.next().equalsIgnoreCase("y");
        if (export) {
            dumpSkills();
            for (Pair c : a) {
                System.out.println("特殊字符： " + c.getRight() + " 技能： " + c.getLeft());
            }
        }
        while (!export) {
            System.out.println("请输入技能ID：");
            int skill;
            try {
                skill = input.nextInt();
            } catch (Exception ex) {
    ...
    public static void dumpSkills() {
        ...
        for (MapleDataFileEntry data : root.getFiles()) {
            if (data.getName().length() > 9) {
                continue;
            }
            System.out.println("导出职业： " + data.getName().replaceAll(".img", ""));
            for (MapleData sub : prov.getData(data.getName())) {
                ...
                for (MapleData sub2 : sub.getChildren()) {
                    for (MapleData sub3 : sub2.getChildren()) {
                        ...
                        if (!found) {
                            System.out.println("新的类型没有找到： " + sub3.getName());
                        }
                        for (MapleData sub4 : sub3.getChildren()) {
                            String tab = "";
                            String name = sub3.getName() + "/" + sub4.getName();
                    ...
                    if (!sb.toString().isEmpty()) {
                        try {
                            ...
                            System.out.println("无法获取信息： " + sub2.getName());
                        }
        ...
        System.out.println("Total time: " + minutes + " minute(s), " + seconds + " second(s), " + milliseconds + " millisecond(s).");
    }
