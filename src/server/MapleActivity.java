...
public class MapleActivity {
    ...
    public static final void initAllActivity() {

        PreparedStatement ps = null;
        try {
            Connection con = DatabaseConnection.getConnection();
            ps = con.prepareStatement("DELETE FROM missionstatus WHERE missionid >= 120101 AND missionid <= 120114");
            ps.executeUpdate();
            ps.close();
        } catch (SQLException ex) {
            FileoutputUtil.log(FileoutputUtil.Acc_Stuck, "Error MissionDelete:" + ex.toString());
        } finally {
            try {
                if (ps != null) {
                    ps.close();
                ...
                java.util.logging.Logger.getLogger(MapleCharacter.class.getName()).log(Level.SEVERE, null, ex);
            }
        }
    }

    public static final void loginTip(MapleClient c) {
        // 发送角色头顶信息
        ...
        c.getSession().write(MaplePacketCreator.sendHint(message, 150, 60));
    }

    public static final int getActivity(final MapleCharacter player) {
        int ret = 0;
        for (QuestActivity q : QuestActivity.values()) {
    ...
    public static final void finish(final MapleCharacter player, int questid) {
    ...
    public static final int getMaxActivity() {
        return activity_max;
    }

    public static final int getDiffActivity(final MapleCharacter player) {
        return activity_max - getActivity(player);
    }

    public static final int getNextStage(final MapleCharacter player) {
        int stage_ = 0;
    ...
    public static final int getNextStageNeed(final MapleCharacter player) {
        return stage[Math.min(stage.length - 1, getNextStage(player) - 1)] - getActivity(player);
    }

    public static final int getRecevieReward(final MapleCharacter player) {
        final int activity = getActivity(player);
