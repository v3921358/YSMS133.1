            ...
            for (QuestItem item : items) {
                if (item.itemid == itemid) {
                    if (!chr.haveItem(item.itemid, item.count, true, false)) {
                        MapleInventoryManipulator.addById(chr.getClient(), item.itemid, (short) item.count, "Obtained from quest (Restored) " + quest.getId() + " on " + FileoutputUtil.CurrentReadable_Date());
                    }
                    return true;
                }
            }
    ...
    public void runStart(MapleCharacter chr, Integer extSelection) {
        ...
        switch (type) {
            ...
            case item:
                ...
                for (QuestItem item : items) {
                    ...
                    if (count < 0) { // 删除任务道具
                        ...
                        MapleInventoryManipulator.addById(chr.getClient(), id, count, "", null, period, "任务获得 " + quest.getId() + " 时间: " + FileoutputUtil.CurrentReadable_Date());
                        chr.getClient().getSession().write(MaplePacketCreator.getShowItemGain(id, count, true));
                    }
                }
                break;
            case nextQuest:
                ...
                chr.getClient().getSession().write(MaplePacketCreator.updateQuestInfo(quest.getId(), status.getNpc(), intStore, false));
                break;
            case money:
                status = chr.getQuest(quest);
                if (status.getForfeited() > 0) {
...
//		MapleQuest.getInstance(intStore).forceComplete(c, 0);
    ...
    public void runEnd(MapleCharacter chr, Integer extSelection) {
        switch (type) {
            ...
            case nextQuest: {
                chr.getClient().getSession().write(MaplePacketCreator.updateQuestInfo(quest.getId(), chr.getQuest(quest).getNpc(), intStore, false));
                break;
            }
            case money: {
                chr.gainMeso(intStore, true, true);
