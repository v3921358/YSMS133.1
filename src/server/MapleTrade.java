...
public class MapleTrade {
    ...
    private List<Item> items = new LinkedList<>(); //保存交易中的道具
    ...
    public void CompleteTrade() {
        ...
        exchangeMeso = 0;
        chr.get().getClient().getSession().write(TradePacket.TradeMessage(tradingslot, (byte) 0x08));
    }
    ...
    public void cancel(MapleClient c, MapleCharacter chr, int message) {
        ...
        meso = 0;
        c.getSession().write(TradePacket.getTradeCancel(tradingslot, message));
    }
    ...
    public void setMeso(int meso) {
        ...
        if (chr.get().getMeso() >= meso) {
            ...
            this.meso += meso;
            chr.get().getClient().getSession().write(TradePacket.getTradeMesoSet((byte) 0, this.meso));
            if (partner != null) {
                partner.getChr().getClient().getSession().write(TradePacket.getTradeMesoSet((byte) 1, this.meso));
            }
        }
    }
    ...
    public void addItem(Item item) {
        ...
        items.add(item);
        chr.get().getClient().getSession().write(TradePacket.getTradeItemAdd((byte) 0, item));
        if (partner != null) {
            partner.getChr().getClient().getSession().write(TradePacket.getTradeItemAdd((byte) 1, item));
        }
    }
    ...
    public void chat(String message) {
        if (!CommandProcessor.processCommand(chr.get().getClient(), message, CommandType.TRADE)) {
            ...
            if (partner != null) {
                partner.getChr().getClient().getSession().write(PlayerShopPacket.playerInterChat(chr.get().getName() + " : " + message, 1));
            }
        }
    ...
    public void chatAuto(String message) {
        ...
        if (partner != null) {
            partner.getChr().getClient().getSession().write(PlayerShopPacket.playerInterChat(message, 1));
        }
        if (chr.get().getClient().isMonitored()) { //Broadcast info even if it was a command.
    ...
    public static void startTrade(MapleCharacter player) {
        if (player.getTrade() == null) {
            ...
            player.getClient().getSession().write(MaplePacketCreator.serverNotice(5, "不能同时做多件事情。"));
        }
    }
    ...
    public static void inviteTrade(MapleCharacter player, MapleCharacter target) {
        ...
        if (target != null && target.getTrade() == null) {
            ...
            player.getClient().getSession().write(MaplePacketCreator.serverNotice(5, "对方正在和其他玩家进行交易中。"));
    ...
    public static void visitTrade(MapleCharacter player, MapleCharacter target) {
        if (target != null && player.getTrade() != null && player.getTrade().getPartner() == target.getTrade() && target.getTrade() != null && target.getTrade().getPartner() == player.getTrade()) {
            ...
            target.getClient().getSession().write(PlayerShopPacket.playerInterVisitorAdd(player, 1));
            ...
            player.getClient().getSession().write(MaplePacketCreator.serverNotice(5, "对方已经取消了交易。"));
        }
    }
