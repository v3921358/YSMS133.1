...
public class MapleInventoryManipulator {
    ...
    public static byte addId(MapleClient c, int itemId, short quantity, String owner, MaplePet pet, long period, int state, String gmLog) {
        ...
        if (!type.equals(MapleInventoryType.EQUIP)) { //如果不是装备道具
                    ...
                    while (quantity > 0) {
                        if (i.hasNext()) {
                            ...
                            if (oldQ < slotMax && (eItem.getOwner().equals(owner) || owner == null) && eItem.getExpiration() == -1) {
                                ...
                                c.getSession().write(InventoryPacket.modifyInventory(true, Collections.singletonList(new ModifyInventory(1, eItem))));
            ...
            if (quantity == 1) {
                ...
                if (state > 0) { //设置装备潜能
                    ii.setOptentialState((Equip) nEquip, state);
                }
        ...
        if (!type.equals(MapleInventoryType.EQUIP)) {
            ...
            if (!ItemConstants.isRechargable(item.getItemId())) {
                    ...
                    while (quantity > 0) {
                        if (i.hasNext()) {
                            ...
                            if (oldQ < slotMax && item.getOwner().equals(eItem.getOwner()) && item.getExpiration() == eItem.getExpiration()) {
                                ...
                                c.getSession().write(InventoryPacket.modifyInventory(updateTick, Collections.singletonList(new ModifyInventory(1, eItem))));
                            }
                        } else {
                            break;
                        }
                ...
                while (quantity > 0) {
                    ...
                    c.getSession().write(InventoryPacket.modifyInventory(updateTick, Collections.singletonList(new ModifyInventory(0, nItem))));
                }
            } else {
                // Throwing Stars and Bullets - Add all into one slot regardless of quantity.
            ...
            if (quantity == 1) {
                ...
                c.getSession().write(InventoryPacket.modifyInventory(updateTick, Collections.singletonList(new ModifyInventory(0, item))));
                if (ItemConstants.isHarvesting(item.getItemId())) {
                    c.getPlayer().getStat().handleProfessionTool(c.getPlayer());
                }
                c.getPlayer().checkCopyItems();
    ...
    public static boolean addItemAndEquip(MapleClient c, int itemId, short slot, int state, boolean removeItem) {
        return addItemAndEquip(c, itemId, slot, null, 0, state, "系统赠送 时间: " + FileoutputUtil.CurrentReadable_Date(), removeItem);
    }
    ...
    private static Item checkEnhanced(Item before, MapleCharacter chr) {
        if (before instanceof Equip) {
            ...
            if (eq.getState() == 0 && (eq.getUpgradeSlots() >= 1 || eq.getLevel() >= 1) && ItemConstants.canScroll(eq.getItemId()) && Randomizer.nextInt(100) >= 90) { //20% chance of pot?
                eq.resetOptential();
                //chr.dropMessage(5, "You have obtained an item with hidden Optential.");
            }
        }
        return before;
    }
    ...
    public static boolean removeFromSlot(MapleClient c, MapleInventoryType type, short slot, short quantity, boolean fromDrop, boolean consume) {
        ...
        if (item != null) {
            ...
            if (item.getQuantity() == 0 && !allowZero) {
                c.getSession().write(InventoryPacket.modifyInventory(fromDrop, Collections.singletonList(new ModifyInventory(3, item))));
            } else {
                c.getSession().write(InventoryPacket.modifyInventory(fromDrop, Collections.singletonList(new ModifyInventory(1, item))));
            }
            return true;
        }
        return false;
    ...
    public static void removeAll(MapleClient c, MapleInventoryType type) {
        ...
        if (!mods.isEmpty()) {
            c.getSession().write(InventoryPacket.modifyInventory(false, mods));
        }
        ...
        for (Item item : copyEquipItems) {
            if (item != null) {
                if (!locked) {
                    ...
                    FileoutputUtil.log(FileoutputUtil.复制装备, msgtext + " 道具唯一ID: " + item.getEquipOnlyId());
                    locked = true;
                } else {
        ...
        if (dst > c.getPlayer().getInventory(type).getSlotLimit()) {
            if (type == MapleInventoryType.ETC && dst > 100 && dst % 100 != 0) {
                ...
                if (eSlot > 0) {
                    ...
                    if (dst % 100 > itemEffect.getSlotCount() || itemEffect.getType() <= 0 || itemEffect.getType() != ii.getBagType(source.getItemId())) {
    ...
    public static void equip(MapleClient c, short src, short dst) {
        ...
        if (dst > -1200 && dst < -999 && !ItemConstants.is龙龙装备(source.getItemId()) && !ItemConstants.is机甲装备(source.getItemId())) {
            ...
            if (chr.isAdmin()) {
                chr.dropMessage(5, "穿戴装备 - 2 " + source.getItemId() + " dst: " + dst + " 检测1: " + (dst <= -1200) + " 检测2: " + (dst >= -999 && dst < -99) + " 检测3: " + !stats.containsKey("cash"));
            }
            c.getSession().write(MaplePacketCreator.enableActions());
            return;
        ...
        if (ItemConstants.is龙龙装备(source.getItemId()) && (chr.getJob() < 2200 || chr.getJob() > 2218)) {
            c.getSession().write(MaplePacketCreator.enableActions());
            return;
        }
        if (ItemConstants.is机甲装备(source.getItemId()) && (chr.getJob() < 3500 || chr.getJob() > 3512)) {
            c.getSession().write(MaplePacketCreator.enableActions());
            return;
        }
        ...
        if (stats.get("equipTradeBlock") != null || source.getItemId() / 10000 == 167) { //禁止交易的装备和智能机器人心脏
        ...
        if (source.getState() >= 17) {
            ...
            int[] potentials = {source.getOptential1(), source.getOptential2(), source.getOptential3(), source.getOptential4(), source.getOptential5(), source.getOptential6()};
            for (int i : potentials) {
                if (i > 0) {
                    int itemReqLevel = ii.getReqLevel(source.getItemId());
                    StructItemOption pot = ii.getOptentialInfo(i).get((itemReqLevel - 1) / 10);
                    if (pot != null && pot.get("skillID") > 0) {
                        skills.put(SkillFactory.getSkill(PlayerStats.getSkillByJob(pot.get("skillID"), chr.getJob())), new SkillEntry((byte) 1, (byte) 0, -1));
                    }
                }
            }
            ...
            for (int i : sockets) {
                if (i > 0) {
                    ...
                    if (soc != null && soc.get("skillID") > 0) {
                        skills.put(SkillFactory.getSkill(PlayerStats.getSkillByJob(soc.get("skillID"), chr.getJob())), new SkillEntry((byte) 1, (byte) 0, -1));
                    }
                }
            }
            chr.changeSkillLevel_Skip(skills, true);
        ...
        if (source.getState() >= 17) {
            ...
            for (int i : potentials) {
                if (i > 0) {
                    ...
                    StructItemOption pot = ii.getOptentialInfo(i).get((itemReqLevel - 1) / 10);
                    if (pot != null && pot.get("skillID") > 0) {
                        skills.put(SkillFactory.getSkill(PlayerStats.getSkillByJob(pot.get("skillID"), c.getPlayer().getJob())), new SkillEntry((byte) 0, (byte) 0, -1));
                    }
                }
            }
        ...
        if (source.getSocketState() >= 0x13) {
            ...
            for (int i : sockets) {
                if (i > 0) {
                    StructItemOption soc = ii.getSocketInfo(i);
                    if (soc != null && soc.get("skillID") > 0) {
                        skills.put(SkillFactory.getSkill(PlayerStats.getSkillByJob(soc.get("skillID"), c.getPlayer().getJob())), new SkillEntry((byte) 0, (byte) 0, -1));
                    }
                }
            }
            c.getPlayer().changeSkillLevel_Skip(skills, true);
    ...
    public static boolean drop(MapleClient c, MapleInventoryType type, short src, short quantity, boolean npcInduced) {
        ...
        if (quantity < source.getQuantity() && !ItemConstants.isRechargable(source.getItemId())) {
            ...
            c.getSession().write(InventoryPacket.modifyInventory(true, Collections.singletonList(new ModifyInventory(1, source)))); //发送更新道具数量的封包
            //log.info("[物品] " + c.getPlayer().getName() + " 丢弃道具: " + target.getItemId() + " x " + target.getQuantity() + " - " + ii.getName(target.getItemId()) + " 地图: " + c.getPlayer().getMapId());
            if (ii.isDropRestricted(target.getItemId()) || ii.isAccountShared(target.getItemId())) {
                ...
                if (ItemConstants.isPet(source.getItemId()) || ItemFlag.UNTRADEABLE.check(flag)) {
                    c.getPlayer().getMap().disappearingItemDrop(c.getPlayer(), c.getPlayer(), target, dropPos);
                } else {
                    c.getPlayer().getMap().spawnItemDrop(c.getPlayer(), c.getPlayer(), target, dropPos, true, true);
                }
            ...
            c.getSession().write(InventoryPacket.modifyInventory(true, Collections.singletonList(new ModifyInventory(3, source)))); //发送删除道具的封包
            if (src < 0) {
                c.getPlayer().equipChanged();
            }
            //log.info("[物品] " + c.getPlayer().getName() + " 丢弃道具: " + source.getItemId() + " x " + source.getQuantity() + " - " + ii.getName(source.getItemId()) + " 地图: " + c.getPlayer().getMapId());
