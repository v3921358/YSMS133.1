...
public class PlayerStats implements Serializable {
    ...
    private List<Equip> durabilityHandling = new ArrayList<>(), equipLevelHandling = new ArrayList<>(), sealedEquipHandling = new ArrayList<>();
     ...
     * 超级技能进阶信息的处理
     */
    private Map<Integer, Integer> add_skill_duration = new HashMap<>(); //增加持续时间
    private Map<Integer, Integer> add_skill_attackCount = new HashMap<>(); //增加攻击怪物的次数
    private Map<Integer, Integer> add_skill_targetPlus = new HashMap<>();  //增加攻击怪物的数量
    private Map<Integer, Integer> add_skill_bossDamageRate = new HashMap<>();  //增加攻击BOSS的伤害
    private Map<Integer, Integer> add_skill_dotTime = new HashMap<>();  //对怪物持续伤害BUFF
    private Map<Integer, Integer> add_skill_prop = new HashMap<>();  //概率
    private Map<Integer, Integer> add_skill_coolTimeR = new HashMap<>();  //减少技能的冷却时间
    private Map<Integer, Integer> add_skill_ignoreMobpdpR = new HashMap<>();  //增加技能攻击怪物无视防御
    ...
    public void recalcLocalStats(boolean first_login, MapleCharacter chra) {
        ...
        while (itera.hasNext()) {
            ...
            if (equip.getPosition() == -11) {
                if (ItemConstants.isMagicWeapon(equip.getItemId())) {
                    Map<String, Integer> eqstat = ii.getEquipStats(equip.getItemId());
            ...
            if (equip.getState() >= 17) {
                int[] potentials = {equip.getOptential1(), equip.getOptential2(), equip.getOptential3(), equip.getOptential4(), equip.getOptential5(), equip.getOptential6()};
                for (int i : potentials) {
                    if (i > 0) {
                        int itemReqLevel = ii.getReqLevel(equip.getItemId());
                        //System.err.println("潜能ID: " + i + " 装备等级: " + itemReqLevel + " 潜能等级: " + (itemReqLevel - 1) / 10);
                        soc = ii.getOptentialInfo(i).get((itemReqLevel - 1) / 10);
                        if (soc != null) {
        ...
        bx = SkillFactory.getSkill(船长.船员统帅); //5210012 - 坚忍不拔 - 凭借大副特有的韧劲，永久增加物理、魔法防御力、最大HP和MP。
        bof = chra.getTotalSkillLevel(bx);
        if (bof > 0) {
            eff = bx.getEffect(bof);
            localmaxhp_ += eff.getMaxHpX();
        ...
        switch (chra.getJob()) {
            ...
            case 112: {
                ...
                if (bof > 0) {
                    ...
                    if (斗气效果 != null && 斗气状态 != null) {
                        percent_damage_rate += eff.getDAMRate() * (斗气状态.intValue() - 1);
                        percent_boss_damage_rate += eff.getDAMRate() * (斗气状态.intValue() - 1);
                    }
                }
                ...
                if (bof > 0) {
                    ...
                    Integer 斗气状态 = chra.getBuffedValue(MapleBuffStat.斗气集中);
                    if (斗气效果 != null && 斗气状态 != null) {
                        percent_boss_damage_rate += eff.getW() * (斗气状态.intValue() - 1);
                    }
                }
            ...
            case 122: {
                ...
                if (bof > 0) {
                    ...
                    if (buff != null) {
                        watk += buff.intValue() / 5 * 12;
                        percent_damage += buff.intValue();
                    }
                }
                /*
                 * 圣骑士超级技能
...
//                }
            ...
            case 572: {
                ...
                bx = SkillFactory.getSkill(龙的传人.真气流贯); //5700011 - 侠客秘诀 - 透过特殊锻炼方式永久提升力量,敏捷与体力.另外,有几率发动格挡效果.\r\n
                bof = chra.getTotalSkillLevel(bx);
                if (bof > 0) {
                    eff = bx.getEffect(bof);
                    localstr += eff.getStrX();
            ...
            case 1111:
            case 1112: {
                bx = SkillFactory.getSkill(魂骑士.HP增加);
                bof = chra.getTotalSkillLevel(bx);
                if (bof > 0) {
                    percent_hp += bx.getEffect(bof).getPercentHP();
                }
                 ...
                 * 新的技能
                 */
                bx = SkillFactory.getSkill(魂骑士.灵魂鸣响); //11000023 - 灵魂鸣响 - 修炼灵魂，聆听灵魂的鸣响。
                bof = chra.getTotalSkillLevel(bx);
                if (bof > 0) {
                    eff = bx.getEffect(bof);
                    percent_hp += eff.getPercentHP();
            ...
            case 2112: {
                ...
                bx = SkillFactory.getSkill(战神.全力挥击); //21110002 - 全力挥击 - 将双重重击升级为连续攻击。此外，提高双重重击、三重重击和斗气爆裂的伤害。\n需要技能：#c三重重击20级以上#
                bof = chra.getTotalSkillLevel(bx);
                if (bof > 0) {
                    eff = bx.getEffect(bof);
                    addDamageIncrease(战神.斗气爆裂, eff.getDAMRate());
                    addDamageIncrease(战神.全力挥击_双重重击, eff.getW());
                    addDamageIncrease(战神.全力挥击_三重重击, eff.getY());
                }
                ...
                bx = SkillFactory.getSkill(战神.战神之舞); //21120002 - 战神之舞 - 双重重击和三重重击全部升级为连续攻击。另外，双重重击和三重重击、幻影狼牙的伤害提升至极限。\n前置技能：#c全力挥击10级以上#
                bof = chra.getTotalSkillLevel(bx);
                if (bof > 0) {
                    eff = bx.getEffect(bof);
                    addDamageIncrease(战神.幻影狼牙, eff.getDAMRate());
                }
                ...
                if (bof > 0) {
                    ...
                    addDamageIncrease(战神.终极投掷, eff.getDAMRate());
                }
                /*
                 * 超级技能
                 */
                bx = SkillFactory.getSkill(战神.斗气重生_缩短冷却时间); //21120044 - 斗气重生-缩短冷却时间 - 缩短斗气重生的冷却时间。
                bof = chra.getTotalSkillLevel(bx);
                if (bof > 0) {
                    addCoolTimeReduce(战神.斗气重生, bx.getEffect(bof).getCooltimeReduceR());
                }
                bx = SkillFactory.getSkill(战神.巨熊咆哮_强化); //21120046 - 巨熊咆哮-强化 - 提升巨熊咆哮的伤害。
                bof = chra.getTotalSkillLevel(bx);
                if (bof > 0) {
                    addDamageIncrease(战神.巨熊咆哮, bx.getEffect(bof).getDAMRate());
                    addDamageIncrease(战神.巨熊咆哮_老技能, bx.getEffect(bof).getDAMRate());
                }
                bx = SkillFactory.getSkill(战神.巨熊咆哮_额外攻击); //21120047 - 巨熊咆哮-额外攻击 - 提升巨熊咆哮的攻击次数。
                bof = chra.getTotalSkillLevel(bx);
                if (bof > 0) {
                    addAttackCount(战神.巨熊咆哮, bx.getEffect(bof).getAttackCount());
                    addAttackCount(战神.巨熊咆哮_老技能, bx.getEffect(bof).getAttackCount());
                }
                bx = SkillFactory.getSkill(战神.巨熊咆哮_无视防御); //21120048 - 巨熊咆哮-无视防御 - 提升巨熊咆哮的防御无视率。
                bof = chra.getTotalSkillLevel(bx);
                if (bof > 0) {
                    addIgnoreMobpdpRate(战神.巨熊咆哮, bx.getEffect(bof).getIgnoreMob());
                    addIgnoreMobpdpRate(战神.巨熊咆哮_老技能, bx.getEffect(bof).getIgnoreMob());
                }
                bx = SkillFactory.getSkill(战神.钻石星辰_额外攻击); //21120049 - 钻石星辰-额外攻击 - 提升钻石星辰的攻击次数。
                bof = chra.getTotalSkillLevel(bx);
                if (bof > 0) {
                    addAttackCount(战神.钻石星辰, bx.getEffect(bof).getAttackCount());
                }
                bx = SkillFactory.getSkill(战神.钻石星辰_额外攻击); //21120049 - 钻石星辰-额外攻击 - 提升钻石星辰的攻击次数。
                bof = chra.getTotalSkillLevel(bx);
                if (bof > 0) {
                ...
                bx = SkillFactory.getSkill(战神.钻石星辰_链接强化); //21120050 - 钻石星辰-链接强化 - 提升神圣审判的伤害增加量。\n前置技能：#c钻石星辰1级以上#
                bof = chra.getTotalSkillLevel(bx);
                if (bof > 0) {
                    addDamageIncrease(战神.钻石星辰, bx.getEffect(bof).getDAMRate());
                    addDamageIncrease(战神.战神之审判, bx.getEffect(bof).getDAMRate());
                }
                break;
            }
            case 2200:
            ...
            case 2218: {
                ...
                bx = SkillFactory.getSkill(龙神.魔法屏障); //22131001 - 魔法屏障 - 在一定时间内生成吸收队员所受的部分伤害的保护膜。被动效果:自己的JP最大值提高10%
                bof = chra.getTotalSkillLevel(bx);
                if (bof > 0) {
                    percent_hp += bx.getEffect(bof).getPercentHP();
                }
                 ...
                 * 超级技能
                 */
                bx = SkillFactory.getSkill(龙神.魔龙分身_强化); //22170043 - 魔龙分身-强化 - 提升魔龙分身的伤害。
                bof = chra.getTotalSkillLevel(bx);
                if (bof > 0) {
                    addDamageIncrease(龙神.魔龙分身, bx.getEffect(bof).getDAMRate());
                }
                bx = SkillFactory.getSkill(龙神.魔龙分身_无视防御); //22170045 - 魔龙分身-无视防御 - 提升魔龙分身的防御无视比例。
                bof = chra.getTotalSkillLevel(bx);
                if (bof > 0) {
                    addIgnoreMobpdpRate(龙神.魔龙分身, bx.getEffect(bof).getIgnoreMob());
                }
                bx = SkillFactory.getSkill(龙神.黑暗迷雾_强化); //22170046 - 黑暗迷雾-强化 - 提升黑暗迷雾的伤害。
                bof = chra.getTotalSkillLevel(bx);
                if (bof > 0) {
                    addDamageIncrease(龙神.黑暗迷雾, bx.getEffect(bof).getDAMRate());
                }
                bx = SkillFactory.getSkill(龙神.黑暗迷雾_缩短冷却时间); //22170047 - 黑暗迷雾-缩短冷却时间 - 黑暗迷雾的冷却时间缩短。
                bof = chra.getTotalSkillLevel(bx);
                if (bof > 0) {
                    addCoolTimeReduce(龙神.黑暗迷雾, bx.getEffect(bof).getCooltimeReduceR());
                }
                bx = SkillFactory.getSkill(龙神.黑暗迷雾_分裂伤害); //22170048 - 黑暗迷雾-分裂伤害 - 将黑暗迷雾的伤害减少至4/1，攻击次数提升至4次。
                bof = chra.getTotalSkillLevel(bx);
                if (bof > 0) {
                    addAttackCount(龙神.黑暗迷雾, bx.getEffect(bof).getAttackCount());
                }
                bx = SkillFactory.getSkill(龙神.熊熊烈火_强化); //22170049 - 熊熊烈火-强化 - 提升熊熊烈火的伤害。
                bof = chra.getTotalSkillLevel(bx);
                if (bof > 0) {
                    addDamageIncrease(龙神.熊熊烈火, bx.getEffect(bof).getDAMRate());
                }
                bx = SkillFactory.getSkill(龙神.熊熊烈火_无视防御); //22170050 - 熊熊烈火-无视防御 - 增加熊熊烈火的防御无视比例。
                bof = chra.getTotalSkillLevel(bx);
                if (bof > 0) {
                    addIgnoreMobpdpRate(龙神.熊熊烈火, bx.getEffect(bof).getIgnoreMob());
                }
                break;
            }
            case 2001:
...
//                }
            ...
            case 6512: {
                 ...
                 * 超级技能
                 */
                bx = SkillFactory.getSkill(爆莉萌天使.灵魂汲取_强化); //65120043 - 灵魂吸取-强化 - 提升灵魂吸取的伤害。
                bof = chra.getTotalSkillLevel(bx);
                if (bof > 0) {
                    addDamageIncrease(爆莉萌天使.灵魂汲取_攻击, bx.getEffect(bof).getDAMRate());
                    addDamageIncrease(爆莉萌天使.灵魂汲取, bx.getEffect(bof).getDAMRate());
                }
            ...
            case 11212: {
                 ...
                 * 模式技能处理
                 */
                int buffSourceId = chra.getBuffSource(MapleBuffStat.模式变更);//林之灵.巨熊模式
                ...
                } else if (buffSourceId == 林之灵.雪豹模式) { //雪豹模式 林之灵.雪豹模式
                ...
                } else if (buffSourceId == 林之灵.猛鹰模式) { //猛鹰模式 林之灵.猛鹰模式
                    //todo
                } else if (buffSourceId == 林之灵.猫咪模式) { //猫咪模式 林之灵.猫咪模式
...
//        }
    ...
    private void handleBuffStats(MapleCharacter chra) {
        ...
        if (buff != null) {
            percent_damage_rate += buff.intValue();
            percent_boss_damage_rate += buff.intValue();
        }
        ...
        if (buff != null) {
            ...
            if (comboslevel > 0) {
                ...
                percent_damage_rate += buff.intValue() * effect.getX();
                percent_boss_damage_rate += buff.intValue() * effect.getX();
            }
        }
...
//        }
        ...
        if (effect != null) {
            reduceDamageRate += effect.getT();
        }
        buff = chra.getBuffedValue(MapleBuffStat.魔法屏障);
        if (buff != null) {
            reduceDamageRate += buff.intValue();
        }
        ...
        buff = chra.getBuffedSkill_Y(MapleBuffStat.黑暗领地);
        if (buff != null) {
            percent_damage_rate += buff.intValue();
...
//        }
        ...
        if (buff != null) {
            incMaxDamage += buff.intValue() * 10000000;
        }
        ...
        if (buff != null) {
            ...
            percent_damage_rate += GameConstants.getDiceStat(buff.intValue(), 5);
            percent_boss_damage_rate += GameConstants.getDiceStat(buff.intValue(), 5);
        ...
        buff = chra.getBuffedValue(MapleBuffStat.玛瑙保佑);
        if (buff != null) {
            dodgeChance += buff;
        }
        ...
        //todo ANGEL_AVOID - 回避增加 , ANGEL_JUMP - 跳跃力 , ANGEL_SPEED - 移动速度
        ...
        if (buff != null) {
            percent_ignore_mob_def_rate += buff.intValue();
        }
        ...
        if (buff != null) {
            passive_sharpeye_max_percent += buff.intValue();
        }
        ...
        effect = chra.getStatForBuff(MapleBuffStat.最终审判);
        if (effect != null) {
            passive_sharpeye_rate += effect.getV();
            ASR += effect.getX();
            TER += effect.getY();
        ...
        buff = chra.getBuffedValue(MapleBuffStat.尖兵神秘代码_BOSS伤害);
        if (buff != null) {
            percent_boss_damage_rate += buff.intValue();
        }
        //tod 强健护甲
        ...
        buff = chra.getBuffedValue(MapleBuffStat.神秘代码);
        if (buff != null) {
            percent_boss_damage_rate += buff.intValue();
        }
        buff = chra.getBuffedValue(MapleBuffStat.恶魔超越);
        if (buff != null) {
            hpRecover_Percent -= buff.intValue() / 2;
    ...
    public void checkEquipSealed(MapleCharacter chr, long gain) {
        ...
        chr.getClient().getSession().write(InventoryPacket.modifyInventory(true, mods, chr));
    }
    ...
    private void CalcPassive_SharpEye(MapleCharacter player) {
        ...
        switch (player.getJob()) { // Apply passive Critical bonus
            ...
            case 2217:
            case 2218: {
                critSkill = SkillFactory.getSkill(龙神.魔法爆击); //22140000 - 魔法爆击 - 增加爆击攻击概率和爆击最小伤害。
                critlevel = player.getTotalSkillLevel(critSkill);
                if (critlevel > 0) {
                    this.passive_sharpeye_rate += (short) (critSkill.getEffect(critlevel).getProp());
                    this.passive_sharpeye_min_percent += critSkill.getEffect(critlevel).getCriticalMin();
    ...
    public void calculateMaxBaseDamage(int watk, int lv2damX, int pvpDamage, MapleCharacter chra) {
        if (watk <= 0) {
            ...
            boolean mage = (job >= 200 && job <= 232) || (job >= 1200 && job <= 1212) || (job >= 2200 && job <= 2218) || (job >= 2700 && job <= 2712) || (job >= 3200 && job <= 3212) || (job >= 11200 && job <= 11212);
            switch (weapon) {
                case 弓:
                case 弩:
                case 短枪:
...
//                Equip we1 = (Equip) weapon_item;
            ...
            if (ServerProperties.ShowPacket()) {
                System.out.println("当前攻击: " + localmaxbasedamage + " 攻击加成: " + localmaxbasedamage * (percent_damage / 100.0f));
            }
            localmaxbasedamage += localmaxbasedamage * (percent_damage / 100.0f);
            if (ServerProperties.ShowPacket()) {
                System.out.println("武器类型: " + weapon + " 攻击力: " + watk + " indieDamR: " + percent_damage / 100.0f);
                System.out.println("武器加成: " + weaponDamageMultiplier + " 主要属性: " + mainstat + " 次要属性: " + secondarystat + " 第三属性: " + thirdstat + " 攻击力加成: " + (watk / 100.0f));
                System.out.println("最终攻击: " + localmaxbasedamage + " 武器突破极限: " + getLimitBreak(chra) + " BUFF或者潜能增加上限: " + incMaxDamage);
            }
        }
    }
        ...
        for (Triple<String, String, String> add : additions) {
            ...
            switch (add.getLeft()) {
                case "elemboost":
                    ...
                    break;
                case "mobcategory":
                    //skip the category, thinkings too expensive to have yet another Map<Integer, Integer> for damage calculations
                    if (add.getMid().equals("damage")) {
                        percent_damage_rate += right;
        ...
        for (Entry<Integer, StructSetItemStat> ent : setItemStats.entrySet()) {
            ...
            if (ent.getKey() == setCount) { //必须等于才处理
                ...
                if (setItemStat.option1 > 0 && setItemStat.option1Level > 0) {
                    soc = ii.getOptentialInfo(setItemStat.option1).get(setItemStat.option1Level);
                    if (soc != null) {
                ...
                if (setItemStat.option2 > 0 && setItemStat.option2Level > 0) {
                    soc = ii.getOptentialInfo(setItemStat.option2).get(setItemStat.option2Level);
                    if (soc != null) {
                        localmaxhp_x += soc.get("incMHP");
                        localmaxmp_x += soc.get("incMMP");
                        handleItemOption(soc, chra, first_login, sData);
    ...
    public void handleItemOption(StructItemOption soc, MapleCharacter chra, boolean first_login, Map<Skill, SkillEntry> sData) {
        ...
        if (first_login && soc.get("skillID") > 0) {
            sData.put(SkillFactory.getSkill(getSkillByJob(soc.get("skillID"), chra.getJob())), new SkillEntry((byte) 1, (byte) 0, -1));
        }
    }
    ...
    public boolean setHp(int newhp, boolean silent, MapleCharacter chra) {
        ...
        if (chra != null) {
            ...
            if (oldHp > hp && !chra.isAlive() && chra.playerDead()) {
                chra.getClient().getSession().write(EffectPacket.playerDeadConfirm(1));
            }
        }
        return hp != oldHp;
    }
