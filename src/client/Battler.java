...
public class Battler implements Serializable {
    ...
    public void setStatus(MonsterStatusEffect mse) {
        ...
        if (ability == PokemonAbility.Immunity && stat == MonsterStatus.持续伤害) {
            return;
        }
        if (ability == PokemonAbility.Insomnia && stat == MonsterStatus.恐慌) {
            return;
        }
        if (ability == PokemonAbility.Limber && stat == MonsterStatus.影网) {
            return;
        }
        if (ability == PokemonAbility.MagmaArmor && stat == MonsterStatus.无法移动) {
            return;
        }
        if (ability == PokemonAbility.OwnTempo && stat == MonsterStatus.挑衅) {
            return;
        }
        if (this.status != null) {
            return;
        ...
        } else if (stat == MonsterStatus.持续伤害) {
