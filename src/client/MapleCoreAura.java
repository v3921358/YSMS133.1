...
public class MapleCoreAura {
    ...
    public void saveToDb() {
        try {
            PreparedStatement ps = DatabaseConnection.getConnection().prepareStatement("UPDATE `character_coreauras` SET `level` = ?, `str` = ?, `dex` = ?, `int` = ?, `luk` = ?, `watk` = ?, `magic` = ?, `expiredate` = ? WHERE `characterid` = ?");
            ps.setInt(1, level);
            ps.setInt(2, str);
            ps.setInt(3, dex);
            ps.setInt(4, int_);
