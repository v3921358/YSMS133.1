...
public class MapleCharacterCards {
    ...
    private List<Pair<Integer, Integer>> skills = new ArrayList<>();  // 技能id, 技能等级
    ...
    public void calculateEffects() {
        ...
        for (Entry<Integer, CardData> cardInfo : cards.entrySet()) {
            if (cardInfo.getValue().chrId > 0) { // 检测角色ID大于0才有
                ...
                if (cardInfo.getKey().intValue() < 4) { //卡片的位置也就是第1个卡片组合
                    if (skillData != null) {
                ...
                } else if (cardInfo.getKey().intValue() > 3 && cardInfo.getKey().intValue() < 7) {
                    if (skillData != null) {
        ...
        if (deck1amount == 3 && cardSkillIds1.size() == 3) { //组合技能 需要有3张卡片且是在同1组才有
            ...
            for (int i : uid) {
                skills.add(new Pair<>(i, GameConstants.getCardSkillLevel(reqRank1))); // 组合卡片
            }
        ...
        if (deck2amount == 3 && cardSkillIds2.size() == 3) {
            ...
            for (int i : uid) {
                skills.add(new Pair<>(i, GameConstants.getCardSkillLevel(reqRank2)));
            }
        ...
        if (deck3amount == 3 && cardSkillIds3.size() == 3) {
            ...
            for (int i : uid) {
                skills.add(new Pair<>(i, GameConstants.getCardSkillLevel(reqRank3)));
            }
