...
public class PotionPotHandler {
    ...
    public static void PotionPotUse(LittleEndianAccessor slea, MapleClient c, MapleCharacter chr) {
        ...
        if (chr.getNextConsume() > time) {
            c.getSession().write(InventoryPacket.showPotionPotMsg(0x00, 0x08)); //0x08 被奇怪的气息所围绕，暂时无法使用道具。
            c.getSession().write(MaplePacketCreator.enableActions());
            return;
        }
        ...
        if (potHp == 0 && potMp == 0) {
            ...
            c.getSession().write(InventoryPacket.showPotionPotMsg(0x00, 0x06)); //0x06 这个药剂罐是空的，请再次填充。
            return;
        }
        ...
        c.getSession().write(InventoryPacket.updataPotionPot(potionPot));
        c.getSession().write(InventoryPacket.showPotionPotMsg(usePot ? 0x01 : 0x00, 0x00));
    }
    ...
    public static void PotionPotAdd(LittleEndianAccessor slea, MapleClient c, MapleCharacter chr) {
        ...
        if (itemEffect != null) {
            ...
            if (addHp <= 0 && addMp <= 0) {
                c.getSession().write(InventoryPacket.updataPotionPot(potionPot));
                c.getSession().write(InventoryPacket.showPotionPotMsg(0x00, 0x00)); //0x00 没有提示。
                return;
            }
            if (potionPot.isFull(addHp, addMp)) {
                c.getSession().write(InventoryPacket.updataPotionPot(potionPot));
                c.getSession().write(InventoryPacket.showPotionPotMsg(0x00, 0x02)); //0x02 这个药剂罐已经满了。
                return;
            }
            //开始检测是否满了
            ...
            c.getSession().write(InventoryPacket.updataPotionPot(potionPot));
            c.getSession().write(InventoryPacket.showPotionPotMsg(0x02));
        }
        c.getSession().write(MaplePacketCreator.enableActions());
    }
    ...
    public static void PotionPotIncr(LittleEndianAccessor slea, MapleClient c, MapleCharacter chr) {
        ...
        c.getSession().write(InventoryPacket.updataPotionPot(potionPot));
        c.getSession().write(InventoryPacket.showPotionPotMsg(useItem ? 0x03 : 0x00, useItem ? 0x00 : 0x03));
    }
}
