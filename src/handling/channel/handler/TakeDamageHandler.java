...
public class TakeDamageHandler {
    ...
    public static void TakeDamage(LittleEndianAccessor slea, MapleClient c, MapleCharacter chr) {
        ...
        if (type != -2 && type != -3 && type != -4) { // 不为地图造成的伤害
        ...
        if (damage == -1) {
            ...
            if (type == -1) {
                if (chr.getJob() == 122 && attacker != null && chr.getInventory(MapleInventoryType.EQUIPPED).getItem((byte) -10) != null) {
                    if (chr.getTotalSkillLevel(圣骑士.守护之神) > 0) {
                        ...
                        attacker.applyStatus(chr, new MonsterStatusEffect(MonsterStatus.眩晕, 1, 圣骑士.守护之神, null, false, 0), false, eff.getDuration(), true, eff);
                        fake = 圣骑士.守护之神;
                    }
                } else if ((chr.getJob() == 3611 || chr.getJob() == 3612) && attacker != null) {
                    ...
                    if (effect != null && effect.getSourceId() == 尖兵.双重防御) {
                        ...
                        if (x > 0 && prop > 0) {
                            List<Pair<MapleBuffStat, Integer>> buffstats = new ArrayList<>();
                            buffstats.add(new Pair<>(MapleBuffStat.黑暗高潮, prop));
                            buffstats.add(new Pair<>(MapleBuffStat.伤害吸收, z));
                            chr.setBuffedValue(MapleBuffStat.黑暗高潮, prop);
                            chr.setBuffedValue(MapleBuffStat.伤害吸收, z);
                            int duration = effect.getDuration();
                            chr.getClient().getSession().write(BuffPacket.giveBuff(effect.getSourceId(), duration, buffstats, effect, chr));
                        } else {
            ...
            if (chr.getTotalSkillLevel(fake) <= 0) {
                if (chr.isShowPacket()) {
                    chr.dropMessage(5, "[Fake] 受到伤害: " + damage + " 技能ID: " + skillId + " 反射伤害: " + pDamage + " defType: " + defType);
                }
                return;
            }
        } else if (damage < -1) {
        ...
        if (chr.getStat().dodgeChance > 0 && Randomizer.nextInt(100) < chr.getStat().dodgeChance) { //删除这个 暂时不加
            //c.getSession().write(MaplePacketCreator.showSpecialEffect(0x16));
            //return;
        }
        if (attacker != null && pPhysical && pDamage > 0 && chr.getTotalSkillLevel(skillId) > 0) {
            switch (skillId) {
                case 英雄.愤怒之火:
                    ...
                    if (chr.isShowPacket()) {
                        chr.dropMessage(5, "受到伤害 - 封包解析: " + damage + " 技能吸收倍率: " + eff.getX() / 100.0 + " 被动减少倍率: " + stats.reduceDamageRate / 100.0);
                    }
        ...
        if (damage > 0) {
            ...
            if (chr.getBuffedValue(MapleBuffStat.魔法盾) != null) {
                ...
                if (isDeadlyAttack) {
                    ...
                    if (chr.isShowPacket()) {
                        chr.dropMessage(5, "[火焰斥力] 受到伤害: " + damage + " 减少Hp: " + hploss + " 减少Mp: " + mploss + " 技能减少: " + (effect.getX() / 100.0));
                    }
                    chr.addMPHP(-hploss, -mploss);
                }
        ...
        if (damage < -1 || damage > 200000) {
            FileoutputUtil.log(FileoutputUtil.掉血错误, "玩家[" + chr.getName() + " 职业: " + chr.getJob() + "]掉血错误 - 类型: " + type + " 怪物ID: " + monsteridfrom + " 伤害: " + damage + " fake: " + fake + " direction: " + direction + " oid: " + oid + " offset: " + offset + " 封包:" + slea.toString(true));
            return;
        }
//        c.getSession().write(MaplePacketCreator.enableActions());
