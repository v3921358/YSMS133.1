...
public class StatsHandling {
    ...
    public static void DistributeSP(LittleEndianAccessor slea, MapleClient c, MapleCharacter chr) {
        ...
        int remainingSp;
        if (JobConstants.is新手职业(skillid / 10000) && (skillid % 10000 == 1000 || skillid % 10000 == 1001 || skillid % 10000 == 1002 || skillid % 10000 == 2) || skillid % 10000 == 291) {
    ...
    public static void ResetHyperAP(LittleEndianAccessor slea, MapleClient c, MapleCharacter chr) {
        ...
        Map<Skill, SkillEntry> newList = new HashMap<>();
        for (Entry<Skill, SkillEntry> toRemove : oldList.entrySet()) {
            if (toRemove.getKey().isHyperStat() && chr.getSkillLevel(toRemove.getKey()) > 0) {
                newList.put(toRemove.getKey(), new SkillEntry((byte) 0, (byte) 0, -1));
            }
        }
