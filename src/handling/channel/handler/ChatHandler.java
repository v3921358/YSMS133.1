...
public class ChatHandler {
    ...
    public static void GeneralChat(String text, byte unk, MapleClient c, MapleCharacter chr) {
        if (text.length() > 0 && chr != null && chr.getMap() != null && !CommandProcessor.processCommand(c, text, chr.getBattle() == null ? CommandType.NORMAL : CommandType.POKEMON)) {
            ...
            if (chr.getCanTalk() || chr.isStaff()) {
                ...
                if (chr.getMap().getId() == 910000000 && ServerConfig.chalkboard) {
    ...
    public static void Others(LittleEndianAccessor slea, MapleClient c, MapleCharacter chr) {
        ...
        if (c.isMonitored()) {
            ...
            WorldBroadcastService.getInstance().broadcastGMMessage(MaplePacketCreator.serverNotice(6, "[GM Message] " + MapleCharacterUtil.makeMapleReadable(chr.getName()) + " 在 (" + chattype + ") 中说: " + chattext));
        }
...
//                WorldBuddyService.getInstance().buddyChat(recipients, chr.getId(), chr.getName(), chattext);
            ...
            case 1:
                ...
                WrodlPartyService.getInstance().partyChat(chr.getParty().getPartyId(), chattext, chr.getName());
                break;
            case 2:
                if (chr.getGuildId() <= 0) {
                    break;
...
//                WorldGuildService.getInstance().guildChat(chr.getGuildId(), chr.getName(), chr.getId(), chattext);
                ...
                if (messengerId == 0) { //创建
                    ...
                    if (mode == 0x00) { //好友聊天 直接创建
                        ...
                        if (messenger != null) { //如果随机的聊天招待不为空就加入这个聊天招待
                            ...
                            c.getPlayer().setMessenger(messengerService.createMessenger(messengerPlayer, type, c.getPlayer().isIntern()));
                            c.getSession().write(MessengerPacket.joinMessenger(0xFF)); //发送提示 等待其他玩家加入..
                        }
                    }
                ...
                if (messenger != null) {
                    ...
                    if (target != null) { //在当前频道中找到该玩家
                        if (!target.isIntern() || c.getPlayer().isIntern()) {
                            c.getSession().write(MessengerPacket.messengerNote(input, 0x04, 0x01));
                        ...
                        if (World.isConnected(input)) { //在其他频道中找到该玩家
                            ...
                            c.getSession().write(MessengerPacket.messengerNote(input, 0x04, 0x00));
                        }
                    }
                }
                break;
            case 0x05: // 拒绝别人的邀请
                ...
                if (target != null) { // 如果在当前频道能找到邀请这的角色信息
                    if (target.getMessenger() != null) {
                        target.getClient().getSession().write(MessengerPacket.messengerNote(c.getPlayer().getName(), 0x05, 0x00));
                    }
                } else { // 如果在其他频道
            ...
            case 0x06: // 聊天
                if (messenger != null) {
                    ...
                    if (messenger.isMonitored() && chattext.length() > c.getPlayer().getName().length() + 3) { //name : NOT name0 or name1
                        WorldBroadcastService.getInstance().broadcastGMMessage(MaplePacketCreator.serverNotice(6, "[GM Message] " + MapleCharacterUtil.makeMapleReadable(c.getPlayer().getName()) + "(Messenger: " + messenger.getMemberNamesDEBUG() + ") said: " + chattext));
                    }
                }
                break;
            case 0x09: //对别人增加好感度
                    ...
                    if (targetPlayer != null && targetPlayer.getId() != c.getPlayer().getId() && targetPlayer.getMessenger() != null && targetPlayer.getMessenger().getId() == messenger.getId()) {
                        switch (c.getPlayer().canGiveLove(targetPlayer)) {
                            case 0x00:
                                ...
                                c.getSession().write(MessengerPacket.giveLoveResponse(0x00, c.getPlayer().getName(), targetPlayer.getName()));
                                targetPlayer.getClient().getSession().write(MessengerPacket.giveLoveResponse(0x00, c.getPlayer().getName(), targetPlayer.getName()));
                                break;
                            case 0x01:
                                c.getSession().write(MessengerPacket.giveLoveResponse(0x01, c.getPlayer().getName(), targetPlayer.getName()));
                                break;
                            case 0x02:
                                c.getSession().write(MessengerPacket.giveLoveResponse(0x02, c.getPlayer().getName(), targetPlayer.getName()));
                                break;
                        }
                    }
                }
            ...
            case 0x0B: //查看对方的信息 也就是别人的好感度多少什么的
                if (messenger != null) {
                    ...
                    if (player != null) {
                        ...
                        c.getSession().write(MessengerPacket.messengerNote(name, 0x04, 0x00));
                    }
                }
            ...
            case 0x05: { // Find
                ...
                if (player != null) {
                    if (!player.isIntern() || c.getPlayer().isIntern() && player.isIntern()) {
                        c.getSession().write(WhisperPacket.getFindReplyWithMap(player.getName(), player.getMap().getId(), mode == 0x44));
                    } else {
                    ...
                    if (ch > 0) {
                        ...
                        if (!player.isIntern() || (c.getPlayer().isIntern() && player.isIntern())) {
                            c.getSession().write(WhisperPacket.getFindReply(recipient, (byte) ch, mode == 0x44));
                        } else {
                            c.getSession().write(WhisperPacket.getWhisperReply(recipient, (byte) 0));
                        }
                        return;
                    ...
                    if (ch == -10) {
                        c.getSession().write(WhisperPacket.getFindReplyWithCS(recipient, mode == 0x44));
                    } else if (ch == -20) {
    ...
    public static void ShowLoveRank(LittleEndianAccessor slea, MapleClient c) {
        ...
        switch (mode) {
            case 0x07: //显示冒险岛明星
                c.getSession().write(MessengerPacket.showLoveRank(0x07));
                break;
            case 0x08: //显示每周的冒险岛明星
                c.getSession().write(MessengerPacket.showLoveRank(0x08));
                break;
        }
    }
...
//        PlayerStorage client = ChatServer.getPlayerStorage();
        ...
        c.getSession().write(ChatPacket.getChatLoginAUTH());
        c.getSession().write(ChatPacket.getChatLoginResult(chr.getClient()));
    }
