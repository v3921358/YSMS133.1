...
public class FamilyHandler {
    ...
    public static void FamilyOperation(LittleEndianAccessor slea, MapleClient c) {
        ...
        if (addChr == null) {
            c.getSession().write(FamilyPacket.sendFamilyMessage(0x41)); //角色不在线，或角色名不正确。
        } else if (addChr.getFamilyId() == c.getPlayer().getFamilyId() && addChr.getFamilyId() > 0) {
            c.getSession().write(FamilyPacket.sendFamilyMessage(0x42)); //是同一冒险学院。
        } else if (addChr.getMapId() != c.getPlayer().getMapId()) {
            c.getSession().write(FamilyPacket.sendFamilyMessage(0x45)); //只有在同一地图中的角色才能登录为同学
        } else if (addChr.getSeniorId() != 0) {
            c.getSession().write(FamilyPacket.sendFamilyMessage(0x46)); //已经是其他角色的同学
        } else if (addChr.getLevel() >= c.getPlayer().getLevel()) {
            c.getSession().write(FamilyPacket.sendFamilyMessage(0x47)); //只能将比自己等级低的角色登录为同学
        } else if (addChr.getLevel() < c.getPlayer().getLevel() - 20) {
            c.getSession().write(FamilyPacket.sendFamilyMessage(0x48)); //等级差异超过20，无法登录为同学。
    ...
    public static void DeleteJunior(LittleEndianAccessor slea, MapleClient c) {
        ...
        c.getSession().write(FamilyPacket.sendFamilyMessage(0x01, other.getId())); // 已和(null)诀别。冒险学院关系已经结束。
        c.getSession().write(MaplePacketCreator.enableActions());
    }
    ...
    public static void DeleteSenior(LittleEndianAccessor slea, MapleClient c) {
        ...
        c.getSession().write(FamilyPacket.sendFamilyMessage(0x01, mgc.getId())); // 已和(null)诀别。冒险学院关系已经结束。
        c.getSession().write(MaplePacketCreator.enableActions());
    }
