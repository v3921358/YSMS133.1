...
public class HiredMerchantHandler {
    ...
    public static boolean UseHiredMerchant(MapleClient c, boolean packet) {
        ...
        if (chr.getMap() != null && chr.getMap().allowPersonalShop()) {
            ...
            if (merchant != null) {
                c.getSession().write(PlayerShopPacket.sendTitleBox(0x08, merchant.getMapId(), merchant.getChannel() - 1));
            } else {
                ...
                if (loadItemFrom_Database(chr) == null) {
                    if (packet) {
                        c.getSession().write(PlayerShopPacket.sendTitleBox(0x07));
                    }
                    return true;
                } else {
                    c.getSession().write(PlayerShopPacket.sendTitleBox(0x09));
                }
            }
        }
        return false;
    ...
    public static void MerchantItemStore(LittleEndianAccessor slea, MapleClient c) {
        ...
        switch (operation) {
            ...
            case 0x18: //不取回道具
                ...
                c.getSession().write(PlayerShopPacket.merchItemStore((byte) 0x2D));
                break;
            case 0x1F: //取回道具  V.131修改 以前0x1D
                ...
                switch (checkstatus) {
                    case 1: //金币太多
                        c.getSession().write(PlayerShopPacket.merchItem_Message((byte) 0x25));
                        return;
                    case 2: //背包栏位不足
                        c.getSession().write(PlayerShopPacket.merchItem_Message((byte) 0x28));
                        return;
                    case 3: //道具有数量限制
                        c.getSession().write(PlayerShopPacket.merchItem_Message((byte) 0x26));
                        return;
                }
                if (pack.getMesos() > 0 && chr.getMeso() + pack.getMesos() > ServerProperties.getMaxMeso()) {
                    c.getSession().write(PlayerShopPacket.merchItem_Message((byte) 0x25));
                    return;
                }
                if (deletePackage(chr.getId())) {
                    ...
                    c.getSession().write(PlayerShopPacket.merchItem_Message((byte) 0x24));
                } else {
                    chr.dropMessage(1, "发生了未知错误.");
                }
                break;
    ...
    public static void RemoteStore(LittleEndianAccessor slea, MapleClient c) {
        ...
        if (merchant != null) {
            if (merchant.getChannel() == chr.getClient().getChannel()) {
                ...
                c.getSession().write(PlayerShopPacket.sendTitleBox(0x10, 0, merchant.getChannel() - 1));
            }
        } else {
    ...
    private static int check(MapleCharacter chr, MerchItemPackage pack) {
        if (chr.getMeso() + pack.getMesos() < 0) {
            log.info("[雇佣] " + chr.getName() + " 雇佣取回道具金币检测错误 时间: " + FileoutputUtil.CurrentReadable_Date());
        ...
        for (Item item : pack.getItems()) {
            ...
            if (MapleItemInformationProvider.getInstance().isPickupRestricted(item.getItemId()) && chr.haveItem(item.getItemId(), 1)) {
                log.info("[雇佣] " + chr.getName() + " 雇佣取回道具是否可以捡取错误 时间: " + FileoutputUtil.CurrentReadable_Date());
        ...
        if (chr.getInventory(MapleInventoryType.EQUIP).getNumFreeSlot() < eq || chr.getInventory(MapleInventoryType.USE).getNumFreeSlot() < use || chr.getInventory(MapleInventoryType.SETUP).getNumFreeSlot() < setup || chr.getInventory(MapleInventoryType.ETC).getNumFreeSlot() < etc || chr.getInventory(MapleInventoryType.CASH).getNumFreeSlot() < cash) {
            log.info("[雇佣] " + chr.getName() + " 雇佣取回道具背包空间不够 时间: " + FileoutputUtil.CurrentReadable_Date());
