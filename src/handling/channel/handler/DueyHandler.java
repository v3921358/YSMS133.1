...
//                String AS13Digit = slea.readMapleAsciiString();
                ...
                if (conv == 2) { // Duey
                    c.getSession().write(MaplePacketCreator.sendDuey((byte) 0x0A, loadItems(c.getPlayer())));
                }
                break;
            }
                    ...
                    if (accid != -1) { //当前有这个角色ID的记录 也就是角色存在
                        if (accid != c.getAccID()) { //发送给其他角色的ID和发送者的角色ID不相同
                            ...
                            if (inventId > 0) { //有发送装备的信息
                                ...
                                if (item == null) {
                                    c.getSession().write(MaplePacketCreator.sendDuey((byte) 0x11, null)); //该角色无法接收快递.
                                    return;
                                }
                                ...
                                if (c.getPlayer().getItemQuantity(item.getItemId(), false) >= amount) {
                                    ...
                                    if (!ii.isDropRestricted(item.getItemId()) && !ii.isAccountShared(item.getItemId())) {
                                        if (addItemToDB(item, amount, mesos, c.getPlayer().getName(), getAccIdFromName(recipient, false), recipientOn)) {
                                            ...
                                            c.getSession().write(MaplePacketCreator.sendDuey((byte) 0x13, null)); // 成功发送.
                                        } else {
                                            c.getSession().write(MaplePacketCreator.sendDuey((byte) 0x11, null)); // 该角色无法接收快递.
                                        }
                                    } else {
                            ...
                            c.getSession().write(MaplePacketCreator.sendDuey((byte) 0x0F, null)); // 不能发送给同一帐号的角色.
                        }
                    } else {
                        c.getSession().write(MaplePacketCreator.sendDuey((byte) 0x0E, null)); // 请重新确认接收人的姓名.
                    }
                } else {
                    c.getSession().write(MaplePacketCreator.sendDuey((byte) 0x0C, null)); // 您的金币不够.
                }
                break;
            }
            case 0x05: { // 接收未领取的道具
                ...
                if (dp.getItem() != null && !MapleInventoryManipulator.checkSpace(c, dp.getItem().getItemId(), dp.getItem().getQuantity(), dp.getItem().getOwner())) {
                    c.getSession().write(MaplePacketCreator.sendDuey((byte) 0x16, null)); // 请查看是否有空间.
                    return;
    ...
    private static boolean addItemToDB(Item item, int quantity, int mesos, String sName, int recipientID, boolean isOn) {
        try {
            PreparedStatement ps = DatabaseConnection.getConnection().prepareStatement("INSERT INTO dueypackages (RecieverId, SenderName, Mesos, TimeStamp, Checked, Type) VALUES (?, ?, ?, ?, ?, ?)", DatabaseConnection.RETURN_GENERATED_KEYS);
            ps.setInt(1, recipientID);
            ps.setString(2, sName);
            ps.setInt(3, mesos);
            ps.setLong(4, System.currentTimeMillis());
