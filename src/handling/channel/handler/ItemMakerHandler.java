...
public class ItemMakerHandler {
    ...
    public static void UseBag(LittleEndianAccessor slea, MapleClient c, MapleCharacter chr) {
        ...
        MapleItemInformationProvider ii = MapleItemInformationProvider.getInstance();
        if (toUse == null || toUse.getQuantity() < 1 || toUse.getItemId() != itemId || itemId / 10000 != 433 || !ii.itemExists(toUse.getItemId())) {
            c.getSession().write(MaplePacketCreator.enableActions());
            return;
        }
        ...
        if (firstTime) {
            ...
            if (!ItemFlag.LOCK.check(toUse.getFlag())) {
    ...
    public static void StartHarvest(LittleEndianAccessor slea, MapleClient c, MapleCharacter chr) {
        ...
        if (lastTime + (5000) > System.currentTimeMillis()) {
            c.getSession().write(MaplePacketCreator.harvestMessage(reactor.getObjectId(), MapleEnumClass.HarvestMsg.HARVEST_UNABLE_COLLECT)); //还无法采集。
        } else {
            marr.setCustomData(String.valueOf(System.currentTimeMillis()));
            c.getSession().write(MaplePacketCreator.harvestMessage(reactor.getObjectId(), MapleEnumClass.HarvestMsg.HARVEST_ACTION_START)); //开始采集
    ...
    public static void ProfessionInfo(LittleEndianAccessor slea, MapleClient c, MapleCharacter chr) {
        ...
        if (skillid.startsWith("9200") || skillid.startsWith("9201")) {
            rate = 100;
        } else if (skillid.equalsIgnoreCase("honorLeveling")) {
            c.getSession().write(MaplePacketCreator.updateSpecialStat(skillid, level1, level2, true, chr.getHonourNextExp()));
            return;
        } else if (skillid.equalsIgnoreCase("hyper_shaman")) {
            return;
        } else if (skillid.equalsIgnoreCase("hyper")) {
            ...
            c.getSession().write(MaplePacketCreator.updateSpecialStat(skillid, level1, level2, level1 < 41, unk));
            return;
        } else if (skillid.equalsIgnoreCase("incHyperStat")) {
            rate = 3 + (level1 / 10 - 14);
        } else if (skillid.equals("needHyperStatLv")) {
            rate = GameConstants.needHyperStatLv[level1];
        } else {
            rate = Math.max(0, 100 - ((level1 + 1) - chr.getProfessionLevel(Integer.parseInt(skillid))) * 20);
        }
        c.getSession().write(MaplePacketCreator.updateSpecialStat(skillid, level1, level2, true, rate));
    }
    ...
    public static void CraftComplete(LittleEndianAccessor slea, MapleClient c, MapleCharacter chr) {
        if (!chr.getCheatTracker().canCraftMake()) {
            ...
            WorldBroadcastService.getInstance().broadcastGMMessage(MaplePacketCreator.serverNotice(6, "[GM Message] " + chr.getName() + " ID: " + chr.getId() + " (等级 " + chr.getLevel() + ") 专业技术制作速度异常。地图ID: " + chr.getMapId()));
            return;
        }
        ...
        if (craftID == 92049000) { //分解装备
            ...
            if (Randomizer.nextInt(100) < (newEquip.getUpgradeSlots() > 0 || potentialChance >= 100 ? potentialChance : (potentialChance / 2))) {
                newEquip.resetOptential_Fuse(theLevl > 5, potentialState);
            }
        ...
        if (!showItems.isEmpty()) {
            c.getSession().write(EffectPacket.getShowItemGain(showItems));
        }
    }
