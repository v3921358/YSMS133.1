...
//                MapleItemInformationProvider ii = MapleItemInformationProvider.getInstance();
            ...
            case 雇佣商店_购买道具0026: {
                ...
                if (shop == null || shop.isOwner(chr) || shop instanceof MapleMiniGame || item >= shop.getItems().size()) {
                    c.getSession().write(PlayerShopPacket.Merchant_Buy_Error((byte) 0x0D));
                    c.getSession().write(MaplePacketCreator.enableActions());
                    return;
                }
                MaplePlayerShopItem tobuy = shop.getItems().get(item);
                if (tobuy == null) {
                    c.getSession().write(PlayerShopPacket.Merchant_Buy_Error((byte) 0x0A));
                    c.getSession().write(MaplePacketCreator.enableActions());
                    return;
                }
                ...
                if (check <= 0 || check2 > ServerProperties.getMaxMeso() || check2 <= 0 || check3 > 32767 || check3 < 0) { //This is the better way to check.
                    c.getSession().write(PlayerShopPacket.Merchant_Buy_Error((byte) 0x0D));
                    c.getSession().write(MaplePacketCreator.enableActions());
                    return;
                }
                if (chr.getMeso() - (check2) < 0) {
                    c.getSession().write(PlayerShopPacket.Merchant_Buy_Error((byte) 0x02));
                    c.getSession().write(MaplePacketCreator.enableActions());
                    return;
                }
                if (tobuy.bundles < quantity || (tobuy.bundles % quantity != 0 && ItemConstants.isEquip(tobuy.item.getItemId())) // Buying
                        || chr.getMeso() - (check2) > ServerProperties.getMaxMeso() || shop.getMeso() + (check2) < 0 || shop.getMeso() + (check2) > ServerProperties.getMaxMeso()) {
                    c.getSession().write(PlayerShopPacket.Merchant_Buy_Error((byte) 0x04));
                    c.getSession().write(MaplePacketCreator.enableActions());
                    return;
                }
