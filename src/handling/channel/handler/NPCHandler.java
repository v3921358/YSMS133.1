...
//                    }
            ...
            case 5: { // 脚本任务结束
                ...
                chr.getClient().getSession().write(EffectPacket.showSpecialEffect(0x0E)); // 任务完成
                chr.getMap().broadcastMessage(chr, EffectPacket.showForeignEffect(chr.getId(), 0x0E), false);
                if (chr.isShowPacket()) {
    ...
    public static void Storage(LittleEndianAccessor slea, MapleClient c, MapleCharacter chr) {
        ...
        switch (mode) {
            case 4: { // 取出
                ...
                if (item != null) {
                    ...
                    if (ii.isPickupRestricted(item.getItemId()) && chr.getItemQuantity(item.getItemId(), true) > 0) {
                        c.getSession().write(NPCPacket.getStorageError((byte) 0x0C));
                        return;
                    }
                    //检测取回道具金币是否足够
                    int meso = storage.getNpcId() == 9030100 || storage.getNpcId() == 9031016 ? 1000 : 0;
                    if (chr.getMeso() < meso) {
                        c.getSession().write(NPCPacket.getStorageError((byte) 0x0B)); //取回道具金币不足
                        return;
                    }
                    //检测角色背包是否有位置
                    if (MapleInventoryManipulator.checkSpace(c, item.getItemId(), item.getQuantity(), item.getOwner())) {
                        ...
                        c.getSession().write(NPCPacket.getStorageError((byte) 0x0A)); //发送背包是满的封包
                    }
                } else {
                    //AutobanManager.getInstance().autoban(c, "试图从仓库取出不存在的道具.");
                    log.info("[作弊] " + chr.getName() + " (等级 " + chr.getLevel() + ") 试图从仓库取出不存在的道具.");
                    WorldBroadcastService.getInstance().broadcastGMMessage(MaplePacketCreator.serverNotice(6, "[GM Message] 玩家: " + chr.getName() + " (等级 " + chr.getLevel() + ") 试图从仓库取出不存在的道具."));
                    c.getSession().write(MaplePacketCreator.enableActions());
                }
                break;
            }
            case 5: { // 存入
                ...
                //检测仓库的道具是否已满
                if (storage.isFull()) {
                    c.getSession().write(NPCPacket.getStorageError((byte) 0x11));
                    return;
                }
                //检测角色背包当前道具是否有道具
                ...
                if (chr.getMeso() < meso) {
                    c.getSession().write(NPCPacket.getStorageError((byte) 0x10));
                    return;
                }
                //开始操作保存道具到仓库
            ...
            case 7: { // 金币
                ...
                if ((meso > 0 && storageMesos >= meso) || (meso < 0 && playerMesos >= -meso)) {
                    if (meso < 0 && (storageMesos - meso) < 0) {
                        meso = (int) -(ServerProperties.getMaxMeso() - storageMesos);
                        if ((-meso) > playerMesos) {
                            return;
                        }
                    } else if (meso > 0 && (playerMesos + meso) < 0) {
                        meso = (int) (ServerProperties.getMaxMeso() - playerMesos);
                        if ((meso) > storageMesos) {
                            return;
                        }
                    }
                    if (meso > 0 && chr.getMeso() + meso > ServerProperties.getMaxMeso()) {
