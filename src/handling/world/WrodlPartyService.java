...
public class WrodlPartyService {
    ...
    private Map<Integer, MapleParty> partyList;
    private Map<Integer, MapleExpedition> expedsList;
    private Map<PartySearchType, List<PartySearch>> searcheList;
    private AtomicInteger runningPartyId, runningExpedId;
    ...
    private WrodlPartyService() {
        ...
        for (PartySearchType pst : PartySearchType.values()) {
            searcheList.put(pst, new ArrayList<PartySearch>()); //according to client, max 10, even though theres page numbers ?!
        }
        System.out.println("[世界组队服务] 已经启动...");
    }
            ...
            if (ch > 0) {
                ...
                if (player != null && !player.getName().equalsIgnoreCase(nameFrom)) {
                    ...
                    if (player.getClient().isMonitored()) {
                        WorldBroadcastService.getInstance().broadcastGMMessage(MaplePacketCreator.serverNotice(6, "[GM Message] " + nameFrom + " said to " + player.getName() + " (组队): " + chatText));
                    }
                }
            }
        }
    ...
    public void updatePartySetup(int partyId, PartyOperation operation, String partyName, boolean isHidden) {
        ...
        for (MaplePartyCharacter partychar : party.getMembers()) {
            ...
            if (ch > 0) {
                ...
                if (chr != null) {
                    chr.setParty(party);
                    chr.getClient().getSession().write(PartyPacket.updateParty(chr.getClient().getChannel(), party, operation, null));
                }
            }
        }
    }
