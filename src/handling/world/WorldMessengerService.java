...
public class WorldMessengerService {
    ...
    private AtomicInteger runningMessengerId;
    ...
    private WorldMessengerService() {
        ...
        System.out.println("[世界信息服务] 已经启动...");
    }
    ...
    public void declineChat(String target, String nameFrom) {
        ...
        if (player != null) {
            if (player.getMessenger() != null) {
                player.getClient().getSession().write(MessengerPacket.messengerNote(nameFrom, 0x05, 0x00));
            }
        }
    }
    ...
    public void messengerInvite(String sender, int messengerId, String target, int fromChannel, boolean gm) {
        ...
        if (targetplayer != null && fromplayer != null) {
            if (!targetplayer.isIntern() || gm) {
                ...
                fromplayer.getClient().getSession().write(MessengerPacket.messengerNote(target, 0x04, 0x01));
            } else {
                fromplayer.getClient().getSession().write(MessengerPacket.messengerNote(target, 0x04, 0x00));
            }
        }
    }
