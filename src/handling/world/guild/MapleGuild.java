...
public class MapleGuild implements Serializable {
    ...
    private void buildNotifications() {
        ...
        Iterator<MapleGuildCharacter> toRemove = members.iterator();
...
//            }
    ...
    public int addGuildMember(MapleGuildCharacter newMember) {
        ...
        try {
            ...
            //删除临时列表
            Iterator<MapleGuildCharacter> itr = applyMembers.iterator();
            while (itr.hasNext()) {
    ...
    public void leaveGuild(MapleGuildCharacter mgc) {
        lock.writeLock().lock();
        try {
            Iterator<MapleGuildCharacter> itr = members.iterator();
            while (itr.hasNext()) {
                MapleGuildCharacter mgcc = itr.next();
    ...
    public static MapleGuildResponse sendInvite(MapleClient c, String targetName) {
        ...
        c.getSession().write(MaplePacketCreator.showRedNotice("已邀请'" + targetName + "'加入公会。"));
