...
public class WorldBuddyService {
    ...
    private WorldBuddyService() {
        //todo
        System.out.println("[世界好友服务] 已经启动...");
    }
    ...
    public void buddyChat(int[] recipientCharacterIds, int chrIdFrom, String chatText) {
        ...
        chrFrom.getChatSession().write(ChatPacket.buddyChat(chrFrom.getAccountID(), chrIdFrom, chatText));
...
//                    chr.getClient().getSession().write(MaplePacketCreator.multiChat(nameFrom, chatText, 0));
                    chr.getChatSession().write(ChatPacket.buddyChat(chrFrom.getAccountID(), chrIdFrom, chatText));
                    if (chr.getClient().isMonitored()) {
                        WorldBroadcastService.getInstance().broadcastGMMessage(MaplePacketCreator.serverNotice(6, "[GM Message] " + chrFrom.getName() + " said to " + chr.getName() + " (好友): " + chatText));
                    }
                }
            }
        }
            ...
            if (ch > 0) {
                ...
                if (chr != null) {
                    ...
                    if (ble != null && ble.isVisible()) {
                        ...
                        chr.getClient().getSession().write(BuddyListPacket.updateBuddyChannel(ble.getCharacterId(), mcChannel, ble.getName()));
                    }
                }
            }
        }
                ...
                switch (operation) {
                    case 添加好友:
                        if (buddylist.contains(chrIdFrom)) {
                            ...
                            addChar.getClient().getSession().write(BuddyListPacket.updateBuddylist(buddylist.getBuddies(), 0x18));
                        }
                        break;
                    case 删除好友:
                        if (buddylist.contains(chrIdFrom)) {
