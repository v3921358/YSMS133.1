    ...
    public static void BuyCashItem(LittleEndianAccessor slea, MapleClient c, MapleCharacter chr) {
        ...
        switch (action) {
            case 0x03: { // 购买道具    T071 OK
                ...
                // 是否允许抵用券购买
                if (toCharge == 2 && !c.getChannelServer().ismPointstobuy()) {
                ...
                if (cItem != null) {
                    ...
                    if (cashinfo.isBlockedCashItemId(cItem.getItemId()) || cashinfo.isBlockCashSnId(snCS)) {
                    ...
                    //item.setFlag((byte) 0x30); //设置道具可以交易一次
            ...
            case 0x07: { // 增加仓库容量    T071 OK
                ...
                if (chr.getCSPoints(toCharge) >= (coupon == 2 ? 1100 : 600) && chr.getStorage().getSlots() < (97 - (4 * coupon))) {
...
//                CashItemInfo item = cashinfo.getItem(sn);
            ...
            case 0x0E: { // 商城 => 背包    V.111.1修改 以前0x0E
                ...
                if (chr.getInventory(ItemConstants.getInventoryType(item.getItemId())).addItem(item) != -1) {
                    cs.removeFromInventory(item);
                    c.getSession().write(MTSCSPacket.moveItemToInvFormCs(item));
                    if (chr.isShowPacket()) {
            ...
            case 0x0F: { // 背包 => 商城    V.111.1修改 以前0x0F
                ...
                if (item != null && item.getQuantity() > 0 && item.getUniqueId() > 0 && chr.getCashInventory().getItemsSize() < 100) {
                    ...
                    c.getSession().write(MTSCSPacket.moveItemToCsFromInv(item_, c.getAccID(), sn));
                } else {
            ...
            case 0x20: { // 换购 V.111.1修改 以前为0x1F
                /*
                 * 7F 01
                 * 1D
                 * 01 00 20
            ...
            case 0x28: {
                ...
                if (item == null || !ItemConstants.isEffectRing(item.getItemId()) || chr.getCSPoints(toCharge) < item.getPrice() || msg.length() > 73 || msg.length() < 1) {
                    c.getSession().write(MTSCSPacket.商城错误提示(0x00));
                    return;
                } else if (!item.genderEquals(chr.getGender())) {
                    c.getSession().write(MTSCSPacket.商城错误提示(0x07));
                    return;
                } else if (chr.getCashInventory().getItemsSize() >= 100) {
                    c.getSession().write(MTSCSPacket.商城错误提示(0x18));
                    return;
                } else if (!ii.isCash(item.getItemId())) { //非商城道具
                    AutobanManager.getInstance().autoban(chr.getClient(), "商城非法购买戒指道具.");
                    return;
                }
                if (cashinfo.isBlockedCashItemId(item.getItemId()) || cashinfo.isBlockCashSnId(snCS)) {
                ...
                if (info == null || info.getLeft().intValue() <= 0) {
                    c.getSession().write(MTSCSPacket.商城错误提示(0x07));
                } else if (info.getMid().intValue() == c.getAccID() || info.getLeft().intValue() == chr.getId()) {
                    c.getSession().write(MTSCSPacket.商城错误提示(0x06));
                } else {
                    if (info.getRight().intValue() == chr.getGender() && action == 0x23) {
                        c.getSession().write(MTSCSPacket.商城错误提示(0x1A));
                        return;
                    }
                    int err = MapleRing.createRing(item.getItemId(), chr, partnerName, msg, info.getLeft().intValue(), item.getSN());
                    if (err != 1) {
                        c.getSession().write(MTSCSPacket.商城错误提示(0x01));
                        return;
                    }
                    chr.modifyCSPoints(toCharge, -item.getPrice(), false);
                    c.getSession().write(MTSCSPacket.商城送礼(item.getItemId(), item.getCount(), partnerName));
                ...
                c.getSession().write(MTSCSPacket.updateCouponsInfo(chr)); //刷新下免得卡住
                break;
            }
            case 0x23: { // 购买礼包   V.111.1修改 以前 0x24
                ...
                if (item == null || packageIds == null) {
                    ...
                    c.getSession().write(MTSCSPacket.商城错误提示(0x03));
                    c.getSession().write(MTSCSPacket.updateCouponsInfo(chr));
                    return;
                ...
                for (int i : packageIds) {
                    ...
                    if (cashinfo.isBlockedCashItemId(item.getItemId())) {
                        continue;
                    }
            ...
            case 0x24: { //商城送礼包   V.111.1修改 以前0x25
                ...
                if (info == null || info.getLeft().intValue() <= 0) {
                    c.getSession().write(MTSCSPacket.商城错误提示(0x07)); //请确认角色名是否错误。
                } else if (info.getLeft().intValue() == chr.getId() || info.getMid().intValue() == c.getAccID()) {
                    c.getSession().write(MTSCSPacket.商城错误提示(0x06)); //无法向本人的账号赠送礼物。请用该角色登陆，然后购买。
                } else if (!item.genderEquals(info.getRight().intValue())) {
                    c.getSession().write(MTSCSPacket.商城错误提示(0x08)); //此道具对性别有限制。请确认接收人的性别。
                } else {
                    if (item.getPrice() <= 0) {
                        AutobanManager.getInstance().autoban(chr.getClient(), "商城非法购买礼包道具.");
                        return;
                    ...
                    //chr.dropMessage(1, "您成功的将礼包送给[" + partnerName + "]花费点卷" + item.getPrice() + "点.");
                    c.getSession().write(MTSCSPacket.商城送礼包(item.getItemId(), item.getCount(), partnerName));
                    chr.sendNote(partnerName, partnerName + " 您已收到" + chr.getName() + "送给您的礼物，请进入现金商城查看！");
            ...
            case 0x25: { // 购买任务道具    V.111.1修改 以前0x26
                ...
                if (cashinfo.isBlockedCashItemId(item.getItemId())) {
                    chr.dropMessage(1, CashShopServer.getCashBlockedMsg(item.getItemId()));
                    c.getSession().write(MTSCSPacket.updateCouponsInfo(chr));
                    return;
                }
                if (item.getItemId() == 4031063 || item.getItemId() == 4031191 || item.getItemId() == 4031192) {
                    byte pos = MapleInventoryManipulator.addId(c, item.getItemId(), (short) item.getCount(), null, "商城: 任务物品" + " 在 " + FileoutputUtil.CurrentReadable_Date());
                    if (pos < 0) {
                        c.getSession().write(MTSCSPacket.updateCouponsInfo(chr));
                        return;
                    }
                    ...
                    c.getSession().write(MTSCSPacket.商城购买任务道具(item.getPrice(), (short) item.getCount(), pos, item.getItemId()));
                } else {
                    AutobanManager.getInstance().autoban(chr.getClient(), "商城非法购买任务道具.");
                }
                break;
            }
            case 0x30: { //V.111.1 以前为0x31
                slea.readByte(); //00 未知
                int snCS = slea.readInt();
            ...
            case 0x32: { // 这个很重要 不发送这个包无法购买商城道具 V.111.1修改 以前是0x33 
                c.getSession().write(MTSCSPacket.redeemResponse());
                break;
            }
            case 0x3F: { //在商城中打开箱子 T071修改 以前是0x3E
                ...
                int snCS = boxItemSNs.get(Randomizer.nextInt(boxItemSNs.size()));
                CashItemInfo cItem = cashinfo.getItem(snCS);
                if (cItem != null) {
                    Item item = cs.toItem(cItem);
                    if (item != null && item.getUniqueId() > 0 && item.getItemId() == cItem.getItemId() && item.getQuantity() == cItem.getCount()) {
                        if (chr.getInventory(ItemConstants.getInventoryType(item.getItemId())).addItem(item) != -1) {
                            ...
                            c.getSession().write(MTSCSPacket.商城打开箱子(item, uniqueId));
                        } else {
    ...
    private static void addCashshopLog(MapleCharacter chr, int SN, int itemId, int type, int price, int count, String itemLog) {
        ...
        try {
            PreparedStatement ps = DatabaseConnection.getConnection().prepareStatement("INSERT INTO cashshop_log (accId, chrId, name, SN, itemId, type, price, count, cash, points, itemlog) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
            ...
            FileoutputUtil.outputFileError(FileoutputUtil.PacketEx_Log, e, "玩家: " + chr.getName() + " ID: " + chr.getId() + " 购买商城道具保存日志出错.");
        }
    }
}
